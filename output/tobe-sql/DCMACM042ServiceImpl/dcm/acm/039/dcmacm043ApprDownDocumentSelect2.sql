...
FROM TB_CSPBII100M S1 WHERE T22.ATRZ_DOC_NO = S1.ATRZ_DOC_NO ) ELSE NULL END AS ADT_MTTR_NM , AD.AUD_DOC_NM , '' AS MNG_DPT_NM , F_DPT_INFO(T22.APV_DPT_CD,'4') AS APV_DPT_NM FROM ( SELECT DISTINCT D3.ATRZ_DOC_NO AS ATRZ_DOC_NO /* 결재문서번호 */,
...
FROM TBDCMACM023L WHERE APRVR_ENO = #APRVR_ENO# AND APVR_SEQ <> '1' AND APRVR_KDCD <> '4' AND SUBSTR(APVR_STA_CD,0,1) IN ('2','4') UNION SELECT L.ATRZ_DOC_NO /* 결재문서번호 */,
    /*기안자가 아닌경우 결재한 이력이 있는 경우 문서를 볼수 있게 조회*/ L.TPOF_ATRZ_SECD AS TPOF_ATRZ_SECD /* 제보결재구분코드 */
FROM TBDCMACM023L L ,TBDCMACM023H H WHERE L.ATRZ_DOC_NO = H.ATRZ_DOC_NO AND L.APRVR_ENO = H.APRVR_ENO AND L.APRVR_ENO = #APRVR_ENO# AND L.APVR_SEQ <> '1' AND L.APRVR_KDCD <> '4' /*임시 조치*/ AND SUBSTR(H.APVR_STA_CD,0,1) IN ('2','4') ) D3 , (SELECT D1.ATRZ_DOC_NO /* 결재문서번호 */,
...
FROM TBFDMADM002M WHERE ADT_DOC_SECD = A.SLCT_RPTP_KDCD GROUP BY ADT_DOC_SECD) END,',') AS AUD_DOC_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID FROM TBBADDED103M A LEFT OUTER JOIN TBBADDED001M C ON (A.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND A.ADT_NO = C.ADT_NO) WHERE A.ATRZ_DMND_ID IS NOT NULL GROUP BY A.CVLCPT_ADT_YR, A.ADT_NO, C.ADT_KDCD, C.ADT_MTTR_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID) OTSD_EXPRT_ADDR WHERE T22.ATRZ_DOC_NO =T23.ATRZ_DOC_NO AND T22.TPOF_ATRZ_SECD = T23.TPOF_ATRZ_SECD AND T22.ATRZ_DOC_NO = AD.ATRZ_DMND_ID(+) AND T22.TPOF_ATRZ_SECD IN ('fieldAudit','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','badear001','DspRqClosingSend') AND T22.TPOF_ATRZ_SECD NOT IN ('fieldAudit','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','DspRqClosingSend') AND T22.ATRZ_STTS_SECD <> '3' /*진행중인 문서만 조회하도록 추가*/ AND T22.SBM_DH BETWEEN #strSbmDyFrom#||'000000' AND #strSbmDyTo#||'235959' T22.APV_TIT_NM LIKE '%'||#strApvTit#||'%' T22.ATRZ_STTS_SECD = #ATRZ_STTS_SECD# T22.TPOF_ATRZ_SECD IN ('fieldAudit','badear001') T22.TPOF_ATRZ_SECD = #TPOF_ATRZ_SECD# ORDER BY T22.ATRZ_STTS_SECD, T22.SBM_DH DESC