...
FROM TBFDMADM002M WHERE ADT_DOC_SECD = A.SLCT_RPTP_KDCD GROUP BY ADT_DOC_SECD) END,',') AS AUD_DOC_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID FROM TBBADDED103M A LEFT OUTER JOIN TBBADDED001M C ON (A.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND A.ADT_NO = C.ADT_NO) WHERE A.ATRZ_DMND_ID IS NOT NULL GROUP BY A.CVLCPT_ADT_YR, A.ADT_NO, C.ADT_KDCD, C.ADT_MTTR_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID) OTSD_EXPRT_ADDR WHERE T22.ATRZ_DOC_NO =T23.ATRZ_DOC_NO(+) AND T22.TPOF_ATRZ_SECD = T23.TPOF_ATRZ_SECD(+) AND T22.SBM_CNB = #APRVR_ENO# AND T22.SBM_DH BETWEEN #strSbmDyFrom#||'000000' AND #strSbmDyTo#||'235959' /*AND T22.APV_STA_CD = '3' 승인된 문서만 조회하도록 추가*/ AND T22.ATRZ_DOC_NO = AD.ATRZ_DMND_ID(+) AND T22.TPOF_ATRZ_SECD IN ('fieldAudit','fieldAuditMain','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','badear001','DspRqClosingSend','AudExecPlanMng','AudInfoSend3') AND T22.TPOF_ATRZ_SECD NOT IN ('fieldAudit','fieldAuditMain','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','DspRqClosingSend','AudExecPlanMng','AudInfoSend3') T22.APV_TIT_NM LIKE '%'||#strApvTit#||'%' T22.ATRZ_STTS_SECD = #ATRZ_STTS_SECD# T22.TPOF_ATRZ_SECD IN ('fieldAudit','badear001','fieldAuditMain') T22.TPOF_ATRZ_SECD = #TPOF_ATRZ_SECD# ORDER BY T22.SBM_DH DESC