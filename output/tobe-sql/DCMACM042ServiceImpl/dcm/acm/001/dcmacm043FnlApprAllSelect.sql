...
FROM TBDCMACM023L WHERE SUBSTR(APVR_STA_CD,0,1) IN ('2','4') ) D3 , (SELECT D1.ATRZ_DOC_NO /* 결재문서번호 */,
...
FROM TBFDMADM002M WHERE ADT_DOC_SECD = A.SLCT_RPTP_KDCD GROUP BY ADT_DOC_SECD) END,',') AS AUD_DOC_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID FROM TBBADDED103M A LEFT OUTER JOIN TBBADDED001M C ON (A.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND A.ADT_NO = C.ADT_NO) WHERE A.ATRZ_DMND_ID IS NOT NULL GROUP BY A.CVLCPT_ADT_YR, A.ADT_NO, C.ADT_KDCD, C.ADT_MTTR_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID) OTSD_EXPRT_ADDR WHERE T22.ATRZ_DOC_NO =T23.ATRZ_DOC_NO AND T22.TPOF_ATRZ_SECD = T23.TPOF_ATRZ_SECD AND T22.ATRZ_DOC_NO = AD.ATRZ_DMND_ID(+) AND T22.ATRZ_STTS_SECD IN ('3','4') /*승인된 문서만 조회하도록 추가*/ AND T22.SBM_DH BETWEEN #strSbmDyFrom#||'000000' AND #strSbmDyTo#||'235959' T22.APV_TIT_NM LIKE '%'||#strApvTit#||'%' T22.ATRZ_STTS_SECD = #ATRZ_STTS_SECD# T22.TPOF_ATRZ_SECD IN ('fieldAudit','fieldAuditMain') T22.TPOF_ATRZ_SECD = #TPOF_ATRZ_SECD# ORDER BY T22.CRU_LAST_ATRZ_DT DESC