...
FROM TB_BADEAR001M D1 ,TB_BADEAR002D D2 WHERE D1.CVLCPT_ADT_YR = D2.ADT_YR AND D1.ADT_NO = D2.ADT_NO AND D1.DSPS_RQT_SQNO = D2.DSPS_RQT_SQNO AND D2.ATRZ_DOC_NO = T22.ATRZ_DOC_NO) ELSE NULL END AS ADT_MTTR_NM , AD.AUD_DOC_NM , '' AS MNG_DPT_NM , F_DPT_INFO(T22.APV_DPT_CD,'4') AS APV_DPT_NM FROM ( SELECT DISTINCT D3.ATRZ_DOC_NO AS ATRZ_DOC_NO /* 결재문서번호 */,
    D3.TPOF_ATRZ_SECD /* 제보결재구분코드 */
FROM ( SELECT ATRZ_DOC_NO /* 결재문서번호 */,
    /*기안한 문서는 반려, 상신취소, 상신완료*/ TPOF_ATRZ_SECD AS TPOF_ATRZ_SECD /* 제보결재구분코드 */
FROM TBDCMACM023L WHERE APRVR_ENO = #APRVR_ENO# AND APVR_SEQ = '1' AND APVR_STA_CD IN ('101','201','301') AND APRVR_JBPS_SECD = '0055700' UNION SELECT ATRZ_DOC_NO /* 결재문서번호 */,
    /*기안자가 아닌경우 결재처리(반려포함)한 문서 기준으로 조회, 협조는 진행문서함에 조회 하지 않는다.*/ TPOF_ATRZ_SECD AS TPOF_ATRZ_SECD /* 제보결재구분코드 */
FROM TBDCMACM023L WHERE APRVR_ENO = #APRVR_ENO# AND APVR_SEQ <> '1' /*임시 조치*/ AND APRVR_KDCD <> '4' AND SUBSTR(APVR_STA_CD,0,1) = '2' AND APRVR_JBPS_SECD = '0055700' UNION SELECT L.ATRZ_DOC_NO /* 결재문서번호 */,
    /*기안자가 아닌경우 결재한 이력이 있는 경우 문서를 볼수 있게 조회*/ L.TPOF_ATRZ_SECD AS TPOF_ATRZ_SECD /* 제보결재구분코드 */
FROM TBDCMACM023L L ,TBDCMACM023H H WHERE L.ATRZ_DOC_NO = H.ATRZ_DOC_NO AND L.APRVR_ENO = H.APRVR_ENO AND L.APRVR_ENO = #APRVR_ENO# AND L.APVR_SEQ <> '1' /*임시 조치*/ AND L.APRVR_KDCD <> '4' /*임시 조치*/ AND L.APRVR_JBPS_SECD = '0055700' AND SUBSTR(H.APVR_STA_CD,0,1) = '2' ) D3 , (SELECT D1.ATRZ_DOC_NO /* 결재문서번호 */,
...
FROM TBFDMADM002M WHERE ADT_DOC_SECD = A.SLCT_RPTP_KDCD GROUP BY ADT_DOC_SECD) END,',') AS AUD_DOC_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID FROM TBBADDED103M A LEFT OUTER JOIN TBBADDED001M C ON (A.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND A.ADT_NO = C.ADT_NO) WHERE A.ATRZ_DMND_ID IS NOT NULL GROUP BY A.CVLCPT_ADT_YR, A.ADT_NO, C.ADT_KDCD, C.ADT_MTTR_NM, C.MNG_DEPT_CD, A.ATRZ_DMND_ID) OTSD_EXPRT_ADDR WHERE T22.ATRZ_DOC_NO =T23.ATRZ_DOC_NO AND T22.TPOF_ATRZ_SECD = T23.TPOF_ATRZ_SECD AND T22.ATRZ_DOC_NO = AD.ATRZ_DMND_ID(+) AND T22.TPOF_ATRZ_SECD IN ('fieldAudit','fieldAuditMain','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','badear001','DspRqClosingSend','AudExecPlanMng') AND T22.TPOF_ATRZ_SECD NOT IN ('fieldAudit','fieldAuditMain','requestAudit1','requestAudit2','requestAudit3','requestAudit4','requestAudit5','requestAudit6','requestAudit7','requestAudit8','DspRqClosingSend','AudExecPlanMng') AND T22.ATRZ_STTS_SECD NOT IN ('3','4','5') /*진행중인 문서만 조회하도록 추가*/ AND T22.SBM_DH BETWEEN #strSbmDyFrom#||'000000' AND #strSbmDyTo#||'235959' T22.APV_TIT_NM LIKE '%'||#strApvTit#||'%' T22.ATRZ_STTS_SECD = #ATRZ_STTS_SECD# T22.TPOF_ATRZ_SECD IN ('fieldAudit','badear001','fieldAuditMain') T22.TPOF_ATRZ_SECD = #TPOF_ATRZ_SECD# ORDER BY T22.ATRZ_STTS_SECD, T22.SBM_DH ASC