SELECT T1.CVLCPT_ADT_YR AS CVLCPT_ADT_YR /* 민원감사년도 */,
    ...
    F_CODE_NM('1000475',T1.PRGRS_STP_SECD) || (CASE WHEN T1.BZTRP_BGT_SECD = '1' AND T1.PRGRS_STP_SECD = '02' THEN (SELECT '/'|| F_CODE_NM('1000030',ADT_BZTRP_KDCD) AS PRGRS_STP_SECD /* 진행단계구분코드 */
FROM TB_BADDED002M WHERE BZTRP_YR || BZTRP_SQNO = (SELECT MAX(BZTRP_YR || BZTRP_SQNO) AS BZTRP_YR /* 출장년도 */
FROM TB_BADDED002M WHERE REL_ADT_YR = T1.CVLCPT_ADT_YR AND REL_ADT_NO = T1.ADT_NO ) ) END) AS PRSS_STEP_NM /*진행단계코드명*/ , T1.GKM_CNT AS GKM_CNT /*감사지식갯수*/ , T1.BZT_CNT AS BZT_CNT , T1.PRTTN_ADT_YR AS PRTTN_ADT_YR /*분할감사년도 - 2015.07.13 yyh*/ , T1.PRTTN_ADT_NO AS PRTTN_ADT_NO /*분할감사번호 - 2015.07.13 yyh*/ , T1.DVS_AUD_SBJ_NM AS DVS_AUD_SBJ_NM /*분할감사사항명 - 2015.07.13 yyh*/ , T1.REF_AUD_INFO AS REF_AUD_INFO /*해당건을 분리전 감사사항으로 사용하는 건 - 2015.07.13 yyh */ , T1.INTG_ADT_YR AS INTG_ADT_YR /*통합감사년도 - 2015.09.15 yyh*/ , T1.INTG_ADT_NO AS INTG_ADT_NO /*통합감사번호 - 2015.09.15 yyh*/ , T1.ITGT_AUD_SBJ_NM AS ITGT_AUD_SBJ_NM /*통합감사사항명 - 2015.09.15 yyh*/ , T1.ITGT_AUD_INFO AS ITGT_AUD_INFO /*해당건을 통합 감사사항으로 사용하는 건 - 2015.09.15 yyh*/ , T1.DVS_AUD_YR_NO_KND_CD AS DVS_AUD_YR_NO_KND_CD , T1.ITGT_AUD_YR_NO_KND_CD AS ITGT_AUD_YR_NO_KND_CD /* 다건의 주관자, 보조자 추가 - 2016.05.03 yyh*/ , F_USR_INFO(T1.AUDTOR1_CNB1,'2') AS AUDTOR1_CNB1 , F_USR_INFO(T1.AUDTOR1_CNB2,'2') AS AUDTOR1_CNB2 , F_USR_INFO(T1.AUDTOR1_CNB3,'2') AS AUDTOR1_CNB3 , F_USR_INFO(T1.AUDTOR2_CNB1,'2') AS AUDTOR2_CNB1 , F_USR_INFO(T1.AUDTOR2_CNB2,'2') AS AUDTOR2_CNB2 , F_USR_INFO(T1.AUDTOR2_CNB3,'2') AS AUDTOR2_CNB3 FROM ( SELECT D1.CVLCPT_ADT_YR AS CVLCPT_ADT_YR /* 민원감사년도 */,
    ...
    F_USR_INFO(D1.ADT_RBPRSN_ENO,'2') AS F_USR_INFO,
    F_DPT_INFO(D1.WHOL_RBPRSN_DEPT_CD,'1') AS F_DPT_INFO,
    F_CODE_NM('1000173',D1.WHOL_RBPRSN_JBGD_SECD) AS F_CODE_NM,
    F_USR_INFO(D1.SPVR_ENO,'2') AS F_USR_INFO,
    F_DPT_INFO(D1.SPVR_DEPT_CD,'1') AS F_DPT_INFO,
    F_CODE_NM('1000173',D1.SPVR_JBGD_SECD) AS F_CODE_NM,
    F_USR_INFO(D1.ASTN_ENO,'2') AS F_USR_INFO,
    F_DPT_INFO(D1.ASTN_DEPT_CD,'1') AS F_DPT_INFO,
    F_CODE_NM('1000173',D1.ASTN_JBGD_SECD) AS F_CODE_NM,
...
FROM TBBADDED001M WHERE INTG_ADT_YR = D1.CVLCPT_ADT_YR AND INTG_ADT_NO = D1.ADT_NO) AS ITGT_AUD_INFO /* 해당건을 통합전 감사사항으로 사용하는 건 - 2015.09.15 yyh */ , D4.AUDTOR1_CNB1 , D4.AUDTOR1_CNB2 , D4.AUDTOR1_CNB3 , D4.AUDTOR2_CNB1 , D4.AUDTOR2_CNB2 , D4.AUDTOR2_CNB3 , D1.BZTRP_BGT_SECD FROM TBBADDED001M D1 , TBCSPDCP101M D2 , TBBADDED003M D3 , (SELECT CVLCPT_ADT_YR /* 민원감사년도 */,
    ADT_NO /* 감사번호 */,
    MAX(CASE WHEN (ADTR_SECD = '1' AND ADTR_SQNO = '1') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */,
    MAX(CASE WHEN (ADTR_SECD = '1' AND ADTR_SQNO = '2') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */,
    MAX(CASE WHEN (ADTR_SECD = '1' AND ADTR_SQNO = '3') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */,
    MAX(CASE WHEN (ADTR_SECD = '2' AND ADTR_SQNO = '1') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */,
    MAX(CASE WHEN (ADTR_SECD = '2' AND ADTR_SQNO = '2') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */,
    MAX(CASE WHEN (ADTR_SECD = '2' AND ADTR_SQNO = '3') THEN TRGT_NOP_ENO ELSE '' END) AS ADTR_SECD /* 감사인구분코드 */
FROM TBBADDED031L WHERE 1=1 GROUP BY CVLCPT_ADT_YR, ADT_NO) D4 WHERE 1=1 AND D1.CVLCPT_RCPT_YR = D2.CVLCPT_RCPT_YR(+) AND D1.CVLCPT_RCPT_RCPT = D2.CVLCPT_RCPT_RCPT(+) AND D1.CLM_SECD = D2.CLM_SECD(+) AND D1.CVLCPT_ADT_YR = D3.CVLCPT_ADT_YR(+) AND D1.ADT_NO = D3.ADT_NO(+) AND D1.CVLCPT_ADT_YR = D4.CVLCPT_ADT_YR(+) AND D1.ADT_NO = D4.ADT_NO(+) /*AND D1.AUD_NO NOT LIKE '9%' 서면모니터링을 걸러내야하므로*/ AND D1.SPRVSN_DEPT_CD IN (SELECT SRECS_DEPT_CD /* 심사재심의부서코드 */
FROM TABLE(F_AUTH_DEPT(#strCnb#, #dptAuth#, '', #strUpSpvBrdvCd#))) /*직급이 원장일 경우 'ALL' 그외는 'DEPTLW'로 셋팅*/ AND D1.CVLCPT_ADT_YR = #CVLCPT_ADT_YR# /* 감사연번종류코드 - 2015.12.17 yyh */ AND SUBSTR(D1.ADT_KDCD,3,1) = #strAudYrNoKndCd# AND D1.ADT_NO = #ADT_NO# AND D1.ADT_MTTR_NM LIKE '%'||#strAudSbjNm#||'%' AND D1.PRGRS_STP_SECD = #PRGRS_STP_SECD# AND D1.ADT_KDCD = #ADT_KDCD# AND D1.SPRVSN_DEPT_CD = #SPRVSN_DEPT_CD# AND D1.CLM_SECD = #CLM_SECD# /*감사사항검색팝업의 경우 서면/모니터링 감사의 경우 통제가 해제된 건에 대해서만 조회되어야 한다. */ AND NOT EXISTS ( SELECT 1
FROM TBBADDED001M WHERE 1=1 AND CVLCPT_ADT_YR = D1.CVLCPT_ADT_YR AND ADT_NO = D1.ADT_NO AND SUBSTR(ADT_KDCD,3,1) = '9' AND NVL(SUBSTR(ADT_MTHD_SECD,3,1),' ') <> '4' ) ) T1 ORDER BY DECODE(SUBSTR(T1.ADT_NO,1,1),'9',2,1),T1.ADT_NO DESC