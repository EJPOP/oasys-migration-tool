SELECT TER.BZTRP_YR /* 출장년도 */,
    ...
    /*출장자 직급명*/ TE.BZTRP_TRVEXP_FDEXP AS BZTRP_TRVEXP_FDEXP /* 출장여비식비 */,
    /*식비*/ TE.LDG_CST AS LDG_CST /* 숙박비용 */,
    /*숙박비*/ TE.BTEXP_CRTR_AMT AS BTEXP_CRTR_AMT /* 출장비기준금액 */,
    /*일비*/ NVL(TE.CPRG_FARE_CST,0) AS CPRG_FARE_CST /* 수도권운임비용 */,
    ...
    /*지급액 - 수도권운임비추가 - 2015.05.06 yyh*/ TE.LDG_DCN AS LDG_DCN /* 숙박일수 */,
    /*숙박일수*/ F_CODE_NM('1000001',TE.RGN_CD) || (SELECT '(' || SUBSTR(F_CODE_NM('1000727',CPRG_DSTNC_SECD),4) || ')' AS RGN_CD /* 지역코드 */
FROM TB_BADDED014B WHERE DPTRE_RGN_SECD = TER.DPTRE_RGN_SECD AND ARVL_RGN_KDCD = TE.RGN_CD AND CPRG_DSTNC_SECD != '90' AND ROWNUM = 1) AS LCL_NM, /*지역명*/ TE.BZTRP_DCN, /*출장일수*/ TE.BZTRP_BGNG_YMD, /*출장기간시작일*/ TE.BZTRP_END_YMD, /*출장기간종료일 */ F_DPT_INFO(BL.OGDP_DEPT_CD,'4') CNSTN_MBCMT_DEPT_NM /*영수인*/ FROM TB_BADDED010L TER, TB_BADDED011L TE, (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
