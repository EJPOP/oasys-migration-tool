...
FROM ( SELECT /* csp.bii.service.impl.CSPBII111QServiceImpl.CSPBII111QMlgInfSelect */ SUM(AA.UTLZ_SCR) + SUM(AA.RLBT_SCR) + SUM(AA.CSIT_SCR) AS UTLZ_SCR /* 활용점수 */,
...
FROM TBCSPBII170M M WHERE M.PRSNR_ENO = A.PRSNR_ENO AND M.ADT_INFO_PRGRS_STTS_SECD IN ('15','20','99') AND M.SBMSN_YMD BETWEEN #strStrSmtDt# AND #strEndSmtDt# AND M.ADT_INFO_PRGRS_STTS_SECD >= '05' AND M.PRCS_DRCTN_SECD = '7' ) AS SMT_DEL_CPT /* 폐기건수 */ , A.PRSNR_ENO FROM TBCSPBII170M A WHERE A.SBMSN_YMD BETWEEN #strStrSmtDt# AND #strEndSmtDt# AND A.PRSNR_ENO = #PRSNR_ENO# AND A.CLCT_SECD IN ('1','2') ) AA WHERE SBMSN_SCR_RNK < 3 UNION ALL SELECT /* csp.bii.service.impl.CSPBII111QServiceImpl.CSPBII111QMlgInfSelect */ SUM(AA.UTLZ_SCR) + SUM(AA.RLBT_SCR) + SUM(AA.CSIT_SCR) AS UTLZ_SCR /* 활용점수 */,
    SUM(AA.UTLZ_SCR) AS UTLZ_WHOL_SCR /* 활용전체점수 */,
    SUM(AA.RLBT_SCR) AS RLBT_WHOL_SCR /* 신뢰성전체점수 */,
    SUM(AA.CSIT_SCR) AS CSIT_TSC /* 충실성총점 */,
    MAX(AA.SMT_DEL_CPT) AS SMT_DEL_CPT /* 폐기건수 */,
