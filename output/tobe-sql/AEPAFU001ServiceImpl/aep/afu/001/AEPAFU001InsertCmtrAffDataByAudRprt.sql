...
FROM (SELECT AA.ADT_YR /* 감사년도 */,
    AA.ADT_NO /* 감사번호 */,
    'A1070' AS ADT_STP_SECD /* 감사단계구분코드 */,
    AA.CVLCPT_DOC_ID /* 문서아이디 */,
    #APRVR_JBPS_SECD# AS APRVR_JBPS_SECD /* 결재자직위구분코드 */,
    AA.INCDNT_SQNO /* 사건순번 */,
    AA.ADT_KDCD /* 감사종류코드 */,
    ...
    AA.CMPTN_YN /* 완료여부 */,
    #copyAudRptId# AS RFRNC_DOC_ID /* 참조문서아이디 */
FROM TBBADDED145M AA , TBBADDED146M CC WHERE AA.CVLCPT_DOC_ID = #CVLCPT_DOC_ID# AND AA.APRVR_JBPS_SECD = #APRVR_JBPS_SECD# AND AA.CVLCPT_ADT_YR = CC.ADT_YR AND AA.ADT_NO = CC.ADT_NO AND AA.CVLCPT_DOC_ID = CC.DOC_ID AND AA.APRVR_JBPS_SECD = CC.APRVR_JBPS_SECD AND AA.INCDNT_SQNO = CC.INCDNT_SQNO AND NOT EXISTS ( SELECT 1
FROM TBBADDED147M ZZ INNER JOIN TBBADDED148M YY ON ZZ.CVLCPT_ADT_YR = YY.CVLCPT_ADT_YR AND ZZ.ADT_NO = YY.ADT_NO AND ZZ.ADT_STP_SECD = YY.ADT_STP_SECD AND ZZ.CVLCPT_DOC_ID = YY.CVLCPT_DOC_ID AND ZZ.APRVR_JBPS_SECD = YY.APRVR_JBPS_SECD AND ZZ.INCDNT_SQNO = YY.INCDNT_SQNO AND ZZ.DSPS_RQT_SQNO = YY.DSPS_RQT_SQNO AND ZZ.MPNG_SQNO = YY.MPNG_SQNO WHERE ZZ.CVLCPT_ADT_YR = AA.CVLCPT_ADT_YR AND ZZ.ADT_NO = AA.ADT_NO AND ZZ.CVLCPT_DOC_ID = AA.CVLCPT_DOC_ID AND ZZ.BFR_INCDNT_NO = F_MNG_KND_AUDYRNO(AA.CVLCPT_ADT_YR, AA.ADT_NO, AA.ADT_KDCD, '-') || '-' || AA.INCDNT_KDCD || '-' || CC.INCDNT_DSPS_TPCD AND ZZ.APRVR_JBPS_SECD = #APRVR_JBPS_SECD# AND YY.INCDNT_MPNG_SECD IN ( 'D90', 'D70', 'D80', 'D99' ) ) ) ) ; INSERT INTO TB_BADDED145M (CVLCPT_ADT_YR, ADT_NO, ADT_STP_SECD, CVLCPT_DOC_ID, APRVR_JBPS_SECD, INCDNT_SQNO, ADT_KDCD, ADT_GRNT_NO, RPTP_FLD_ID, INCDNT_KDCD, LIST_SQNO, SBCN_SECD, INCDNT_FLD_CN, SBCM_AGND_NM, INDCT_TTL, QPB_YN, QPB_YMD, ADTR_NM, DTL_DOC_NO, DATA_INPT_SECD, SNTHS_DPP_SE_YN, SCRT_YN /* 증권여부 */, SRECS_SORT_SQNO, DPP_RVWR_ENO, FRST_USER_ID, LAST_USER_ID, LAST_PRCS_DEPT_CD, LAST_MENU_ID, FRST_REG_DT, LAST_MDFCN_DT, CMPTN_YN, RFRNC_DOC_ID) (SELECT DISTINCT AA.ADT_YR AS ADT_YR /* 감사년도 */,
    AA.ADT_NO /* 감사번호 */,
    'A1070',
    AA.CVLCPT_DOC_ID /* 문서아이디 */,
    #APRVR_JBPS_SECD#,
    ...
    AA.CMPTN_YN /* 완료여부 */,
    #copyAudRptId#
FROM TB_BADDED145M AA , TB_BADDED146M CC WHERE AA.CVLCPT_DOC_ID = #CVLCPT_DOC_ID# AND AA.APRVR_JBPS_SECD = #APRVR_JBPS_SECD# AND AA.CVLCPT_ADT_YR = CC.ADT_YR AND AA.ADT_NO = CC.ADT_NO AND AA.CVLCPT_DOC_ID = CC.DOC_ID AND AA.APRVR_JBPS_SECD = CC.APRVR_JBPS_SECD AND AA.INCDNT_SQNO = CC.INCDNT_SQNO AND NOT EXISTS ( SELECT 1
FROM TB_BADDED147M ZZ INNER JOIN TB_BADDED148M YY ON ZZ.CVLCPT_ADT_YR = YY.CVLCPT_ADT_YR AND ZZ.ADT_NO = YY.ADT_NO AND ZZ.ADT_STP_SECD = YY.ADT_STP_SECD AND ZZ.CVLCPT_DOC_ID = YY.CVLCPT_DOC_ID AND ZZ.APRVR_JBPS_SECD = YY.APRVR_JBPS_SECD AND ZZ.INCDNT_SQNO = YY.INCDNT_SQNO AND ZZ.DSPS_RQT_SQNO = YY.DSPS_RQT_SQNO AND ZZ.MPNG_SQNO = YY.MPNG_SQNO WHERE ZZ.CVLCPT_ADT_YR = AA.CVLCPT_ADT_YR AND ZZ.ADT_NO = AA.ADT_NO AND ZZ.CVLCPT_DOC_ID = AA.CVLCPT_DOC_ID AND ZZ.BFR_INCDNT_NO = F_MNG_KND_AUDYRNO(AA.CVLCPT_ADT_YR, AA.ADT_NO, AA.ADT_KDCD, '-') || '-' || AA.INCDNT_KDCD || '-' || CC.INCDNT_DSPS_TPCD AND ZZ.APRVR_JBPS_SECD = #APRVR_JBPS_SECD# AND YY.INCDNT_MPNG_SECD IN ( 'D90', 'D70', 'D80', 'D99' ) ) ) ; END;