DECLARE BEGIN UPDATE TB_BADDED145M SET CMPTN_YN = 'Y' /* 완료여부 */WHERE CVLCPT_DOC_ID = #CVLCPT_DOC_ID# AND APRVR_JBPS_SECD = (SELECT MAX(B.APRVR_JBPS_SECD) AS APRVR_JBPS_SECD /* 결재자직위구분코드 */
...
FROM TB_BADDED145M WHERE CVLCPT_DOC_ID = #CVLCPT_DOC_ID# AND APRVR_JBPS_SECD = (SELECT MAX(B.APRVR_JBPS_SECD) AS APRVR_JBPS_SECD /* 결재자직위구분코드 */
FROM TB_BADDED103M A , TBDCMACM023L B WHERE B.ATRZ_DOC_NO = #ATRZ_DOC_NO# AND A.ATRZ_DMND_ID = B.ATRZ_DOC_NO AND B.APV_SNO = ( SELECT MAX(D.APV_SNO)
