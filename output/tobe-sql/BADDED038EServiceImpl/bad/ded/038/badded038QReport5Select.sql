...
FROM TBDCMACM013C WHERE CD= SITU.ADT_BZTRP_KDCD AND CMM_CD_DVSN_CD='1000030') END ) ADT_KDCD, SITU.REL_ADT_YR, SITU.REL_ADT_NO, SITU.CVLCPT_ADT_STP_SQNO, /*NVL(APTD.SUM_AUD_DCN, SITU.AUD_DCN) AUD_DCN,*/ SITU.DSPS_RQT_NM, F_DPT_INFO(SITU.BZTRP_DEPT_CD, '4') CNSTN_MBCMT_DEPT_NM, SITU.BZTRP_TL_JBGD_SECD, USR.USER_NM TRIP_LEADER_NM, NVL(F_CODE_NM('1000176',USR.JBPS_CD),F_CODE_NM('1000173',SITU.BZTRP_TL_JBGD_SECD)) TRIP_LEADER_TIT_TXT, USR.JBPS_CD, USR.JBGD_SECD, SITU.BZTRP_DEPT_CD, NVL(PSON.CNT, 0) || CASE WHEN NVL(PSON.EXT_CNT, 0) > 0 THEN CHR(10) || '(차출 등' || PSON.EXT_CNT ||')' END AS PSON, CASE WHEN SUBSTR(SITU.ADT_BGNG_YMD, 5, 1) = '0' THEN SUBSTR(SITU.ADT_BGNG_YMD, 6, 1) ELSE SUBSTR(SITU.ADT_BGNG_YMD, 5, 2) END || '.' || CASE WHEN SUBSTR(SITU.ADT_BGNG_YMD, 7, 1) = '0' THEN SUBSTR(SITU.ADT_BGNG_YMD, 8, 1) ELSE SUBSTR(SITU.ADT_BGNG_YMD, 7, 2) END || '.~' || CASE WHEN SUBSTR(SITU.ADT_END_YMD, 5, 1) = '0' THEN SUBSTR(SITU.ADT_END_YMD, 6, 1) ELSE SUBSTR(SITU.ADT_END_YMD, 5, 2) END || '.' || CASE WHEN SUBSTR(SITU.ADT_END_YMD, 7, 1) = '0' THEN SUBSTR(SITU.ADT_END_YMD, 8, 1) ELSE SUBSTR(SITU.ADT_END_YMD, 7, 2) END || '.' AS AUD_DATE, /*금일국과별현황 출력의 감사착수 오늘 감사기간 수정(박진호감사관, 20250729)*/ F_ORG_INFO(TTO.REP_ORG_CD,'1') REP_ORG_NM FROM TBBADDED002M SITU, TBBADDED001M AUD, TBDCMACM001M USR, (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
