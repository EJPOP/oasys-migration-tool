SELECT F_MNG_KND_AUDYRNO(BASE.REL_ADT_YR, BASE.REL_ADT_NO, BASE.ADT_KDCD, '-') ||'('|| BASE.CVLCPT_ADT_STP_SQNO ||')' AS F_MNG_KND_AUDYRNO /* 감사연번 */,
    BASE.BZTRP_YR /* 출장년도 */,
    BASE.BZTRP_SQNO /* 출장순번 */,
    BASE.ADT_KDCD /* 감사종류코드 */,
    F_CODE_NM('1000007',BASE.ADT_KDCD) AS F_CODE_NM /* 감사종류 */,
    BASE.SPVR_ENO /* 감리원직원전산번호 */,
...
FROM ( SELECT SITU.BZTRP_SECD /* 출장구분코드 */,
    ...
    F_USR_INFO(SITU.BZTRP_TL_ENO,'2') AS F_USR_INFO,
...
FROM (SELECT BZTRP_YR /* 출장년도 */,
    ...
    EMP_SECD /* 직원구분코드 */
FROM TBBADDED005L WHERE 1 = 1 AND BZTRP_BGNG_YMD = #ADT_BGNG_YMD# GROUP BY BZTRP_YR, BZTRP_SQNO, BZTRVR_ENO, EMP_SECD ) AS1 GROUP BY BZTRP_YR, BZTRP_SQNO) PSON, (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
...
FROM (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
    INST_CD /* 기관코드 */
FROM TBBADDED008L GROUP BY BZTRP_YR, BZTRP_SQNO, INST_CD) AS O GROUP BY BZTRP_YR, BZTRP_SQNO) AS TTO WHERE AUD.CVLCPT_ADT_YR = SITU.REL_ADT_YR AND AUD.ADT_NO = SITU.REL_ADT_NO AND TTO.BZTRP_YR(+) = SITU.BZTRP_YR AND TTO.BZTRP_SQNO(+)= SITU.BZTRP_SQNO AND PSON.BZTRP_YR(+) = SITU.BZTRP_YR AND PSON.BZTRP_SQNO(+) = SITU.BZTRP_SQNO AND ORG.INST_CD(+) = TTO.REP_ORG_CD AND SITU.BZTRP_SECD ='1' AND AUD.MNG_DEPT_CD IN (SELECT SRECS_DEPT_CD /* 심사재심의부서코드 */
FROM TABLE(F_AUTH_DEPT(#strCnb#, #strDeptAuth#, ''))) /* 감찰담당관실은 조회시 제외하며, 로그인 한 담당자가 감찰담당관일때만 포함하여 모두 보임 - 2016.01.04 yyh */ AND AUD.SPRVSN_DEPT_CD != '100100' ) BASE ,(SELECT TMA.BZTRP_YR /* 출장년도 */,
    TMA.BZTRP_SQNO /* 출장순번 */,
    TMA.BZTRVR_ENO /* 출장자직원전산번호 */,
    SUM(NVL(PTD.ADT_DCN, 0)) AS ADT_DCN /* 감사일수 */
FROM TBBADDED005L TMA, TBBADDED006L PTD WHERE TMA.BZTRP_YR = PTD.BZTRP_YR AND TMA.BZTRP_SQNO = PTD.BZTRP_SQNO AND TMA.FT_SQNO = PTD.FT_SQNO AND TMA.BZTRVR_ALTMNT_SQNO = PTD.BZTRVR_ALTMNT_SQNO GROUP BY TMA.BZTRP_YR, TMA.BZTRP_SQNO, TMA.BZTRVR_ENO) APTD WHERE 1=1 AND APTD.BZTRP_YR(+) = BASE.BZTRP_YR AND APTD.BZTRP_SQNO(+) = BASE.BZTRP_SQNO AND APTD.BZTRVR_ENO(+) = BASE.SPVR_ENO AND BASE.BZTRP_DEPT_CD = #BZTRP_DEPT_CD# AND BASE.ADT_BGNG_YMD = #ADT_BGNG_YMD# ORDER BY BASE.BZTRP_DEPT_CD