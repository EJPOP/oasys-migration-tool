...
FROM TBDCMACM013C WHERE CD= SITU.ADT_BZTRP_KDCD AND CMM_CD_DVSN_CD='1000030') END ) ADT_KDCD, SITU.REL_ADT_YR, SITU.REL_ADT_NO, SITU.CVLCPT_ADT_STP_SQNO, /*NVL(APTD.SUM_AUD_DCN, SITU.AUD_DCN) AUD_DCN,*/ SITU.DSPS_RQT_NM, F_DPT_INFO(SITU.BZTRP_DEPT_CD, '4') CNSTN_MBCMT_DEPT_NM, SITU.BZTRP_TL_JBGD_SECD, USR.USER_NM TRIP_LEADER_NM, NVL(F_CODE_NM('1000176',USR.JBPS_CD),F_CODE_NM('1000173',SITU.BZTRP_TL_JBGD_SECD)) TRIP_LEADER_TIT_TXT, USR.JBPS_CD, USR.JBGD_SECD, SITU.BZTRP_DEPT_CD, NVL(PSON.CNT, 0) || CASE WHEN NVL(PSON.EXT_CNT, 0) > 0 THEN CHR(10) || '(차출 등' || PSON.EXT_CNT ||')' END AS PSON, CASE WHEN SUBSTR(SITU.ADT_BGNG_YMD, 5, 1) = '0' THEN SUBSTR(SITU.ADT_BGNG_YMD, 6, 1) ELSE SUBSTR(SITU.ADT_BGNG_YMD, 5, 2) END || '.' || CASE WHEN SUBSTR(SITU.ADT_BGNG_YMD, 7, 1) = '0' THEN SUBSTR(SITU.ADT_BGNG_YMD, 8, 1) ELSE SUBSTR(SITU.ADT_BGNG_YMD, 7, 2) END || '.~' || CASE WHEN SUBSTR(SITU.ADT_END_YMD, 5, 1) = '0' THEN SUBSTR(SITU.ADT_END_YMD, 6, 1) ELSE SUBSTR(SITU.ADT_END_YMD, 5, 2) END || '.' || CASE WHEN SUBSTR(SITU.ADT_END_YMD, 7, 1) = '0' THEN SUBSTR(SITU.ADT_END_YMD, 8, 1) ELSE SUBSTR(SITU.ADT_END_YMD, 7, 2) END || '.' AS AUD_DATE, /*금일국과별현황 출력의 감사착수 내일 감사기간 수정(박진호감사관, 20250729)*/ F_ORG_INFO(TTO.REP_ORG_CD,'1') REP_ORG_NM FROM TBBADDED002M SITU, TBBADDED001M AUD, TBDCMACM001M USR, (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
...
FROM (SELECT BZTRP_YR /* 출장년도 */,
    ...
    EMP_SECD /* 직원구분코드 */
FROM TBBADDED005L WHERE BZTRP_BGNG_YMD = TO_CHAR(TO_DATE(#strDate#)+1,'YYYYMMDD') GROUP BY BZTRP_YR, BZTRP_SQNO, BZTRVR_ENO, EMP_SECD) AS1 GROUP BY BZTRP_YR, BZTRP_SQNO) PSON, (SELECT BZTRP_YR /* 출장년도 */,
    BZTRP_SQNO /* 출장순번 */,
    MIN(INST_CD) AS INST_CD /* 기관코드 */
FROM TBBADDED008L GROUP BY BZTRP_YR, BZTRP_SQNO) AS TTO WHERE SITU.REL_ADT_YR = AUD.CVLCPT_ADT_YR AND SITU.REL_ADT_NO = AUD.ADT_NO AND SITU.BZTRP_YR = TTO.BZTRP_YR AND SITU.BZTRP_SQNO = TTO.BZTRP_SQNO AND SITU.BZTRP_YR = PSON.BZTRP_YR(+) AND SITU.BZTRP_SQNO = PSON.BZTRP_SQNO(+) AND SITU.BZTRP_SECD ='1' AND SITU.ADT_BGNG_YMD = TO_CHAR(TO_DATE(#strDate#)+1,'YYYYMMDD') AND SITU.ADT_BZTRP_KDCD != '03' AND USR.TRGT_NOP_ENO(+) = SITU.BZTRP_TL_ENO /* 감찰담당관실은 조회시 제외하며, 로그인 한 담당자가 감찰담당관일때만 포함하여 모두 보임 - 2016.01.04 yyh */ AND AUD.SPRVSN_DEPT_CD != '100100' ) BASE , (SELECT TMA.BZTRP_YR /* 출장년도 */,
    TMA.BZTRP_SQNO /* 출장순번 */,
    TMA.BZTRVR_ENO /* 출장자직원전산번호 */,
    SUM(NVL(PTD.ADT_DCN, 0)) AS ADT_DCN /* 감사일수 */
FROM TBBADDED005L TMA, TBBADDED006L PTD WHERE TMA.BZTRP_YR = PTD.BZTRP_YR AND TMA.BZTRP_SQNO = PTD.BZTRP_SQNO AND TMA.FT_SQNO = PTD.FT_SQNO AND TMA.BZTRVR_ALTMNT_SQNO = PTD.BZTRVR_ALTMNT_SQNO GROUP BY TMA.BZTRP_YR, TMA.BZTRP_SQNO, TMA.BZTRVR_ENO) APTD WHERE BASE.BZTRP_YR = APTD.BZTRP_YR(+) AND BASE.BZTRP_SQNO = APTD.BZTRP_SQNO(+) AND BASE.SPVR_ENO = APTD.BZTRVR_ENO(+)