...
FROM TBBADDED103M A1 WHERE A1.ADT_STP_SECD = 'A1050' AND A1.SLCT_RPTP_KDCD = 'A10500400' AND (A1.ATRZ_STTS_SECD IS NULL OR A1.ATRZ_STTS_SECD AND D1.ADT_MTTR_NM LIKE '%'||#strAudSbjNm#||'%' AND D1.PRGRS_STP_SECD = #PRGRS_STP_SECD# AND D1.ADT_KDCD = #ADT_KDCD# AND D1.SPRVSN_DEPT_CD = #SPRVSN_DEPT_CD# AND D1.CLM_SECD = #CLM_SECD# /*감사사항검색팝업의 경우 서면/모니터링 감사의 경우 통제가 해제된 건에 대해서만 조회되어야 한다. */ AND NOT EXISTS ( SELECT 1
FROM TBBADDED001M WHERE 1=1 AND CVLCPT_ADT_YR = D1.CVLCPT_ADT_YR AND ADT_NO = D1.ADT_NO AND SUBSTR(ADT_KDCD,3,1) = '9' AND NVL(SUBSTR(ADT_MTHD_SECD,3,1),' ') <> '4' ) ) T1 WHERE 1=1 AND T1.ADT_BGNG_YMD >= '20170908' AND T1.ADT_BGNG_YMD AND T1.CRU_LAST_ATRZ_DT &gt;= #strAudEnd_DTS# AND T1.CRU_LAST_ATRZ_DT &lt;= #strAudEnd_DTE# /* 정보보고서 1: 보고 , 2:미보고 */ AND T1.ATRZ_STTS_SECD = '30' AND (T1.ATRZ_STTS_SECD != '30' OR T1.ATRZ_STTS_SECD IS NULL) /*종료보고일 1: 도래 , 2:미도래 */ AND T1.CRU_LAST_ATRZ_DT IS NOT NULL AND T1.CRU_LAST_ATRZ_DT IS NULL ORDER BY T1.CRU_LAST_ATRZ_DT DESC NULLS LAST,T1.ADT_BGNG_YMD DESC )TT /* 20200729 T1.AUD_NO DESC에서 변경 보고일 내림차순, 미완료건중 착수일 내림차순 */