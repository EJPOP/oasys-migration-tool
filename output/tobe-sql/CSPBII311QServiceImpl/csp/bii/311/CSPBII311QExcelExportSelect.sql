...
FROM TBDCMACM015M B WHERE B.INST_CD = A.OGDP_INST_CD) AS TPOF_OGDP_INST_NM /* 소속기관명 */ , A.JBPS_NM || ' / ' || F_CODE_NM('1000173', A.CLSS_CD) AS PST_RANK_NM /* 직위계급명 */ , F_CODE_NM('1000084', A.EPS_MNG_DVSN_CD) AS EPS_MNG_DVSN_NM /* 중점관리구분명 */ , F_CODE_NM('1000403',A.RSG_YN) AS RSG_YN , CASE WHEN (SELECT 1
...
FROM TBCSPBII630L B WHERE B.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY AND ROWNUM = 1) = 1 THEN '특이 ' END AS UNQ_LIST /* 관련감찰자료 */ ,(SELECT F_CODE_NM('1000908',S1.DBT_PSN_DVSN_CD) AS F_CODE_NM
FROM TBCSPBII380M S1 WHERE S1.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY) AS DBT_PSN_DVSN_NM FROM TBCSPBII300M A WHERE 1=1 AND A.INFO_RLPR_NM LIKE #nam#||'%' AND A.CRU_RRNO_SCRTY LIKE #strEin# || '%' AND A.OGDP_INST_CD LIKE SUBSTR(#OGDP_INST_CD#, 0, INSTR(#OGDP_INST_CD#, '000') -1) || '%' AND A.OGDP_INST_CD = #OGDP_INST_CD# AND A.OGDP_INST_CD IN (SELECT B.INST_CD /* 기관코드 */
...
FROM TB_BADEAR001M B , TB_BADEAR007M C WHERE B.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND B.ADT_NO = C.ADT_NO AND B.DSPS_RQT_SQNO = C.DSPS_RQT_SQNO AND B.DSPS_RQT_NM LIKE '%' || #schTxt# || '%' UNION ALL /* 감사정보 관련자 */ SELECT C.RRNO_SCRTY /* 주민등록번호보안 */
FROM TB_CSPBII100M B , TB_CSPBII130L C WHERE B.INFO_YR = C.INFO_YR AND B.FRS_MDM_SQNO = C.FRS_MDM_SQNO AND B.DSPS_RQT_NM LIKE '%' || #schTxt# || '%' UNION ALL /* 자체감사 관련자 */ SELECT C.CRU_RRNO_SCRTY /* 부패주민등록번호보안 */
...
FROM TB_BADGDA001M B , TB_BADGDA001L C WHERE B.INST_CD = C.INST_CD AND B.NTFCTN_SECD = C.NTFCTN_SECD AND B.LINK_YR = C.LINK_YR AND B.SRECS_DEPT_CD = C.SRECS_DEPT_CD AND B.NTFCTN_SQNO = C.NTFCTN_SQNO AND B.RSCH_ASMT_TTL LIKE '%' || #schTxt# || '%' UNION ALL /* 기타정보 관련자 */ SELECT C.CRU_RRNO_SCRTY /* 부패주민등록번호보안 */
...
FROM TB_BADEAR001M B , TB_BADEAR007M C WHERE B.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND B.ADT_NO = C.ADT_NO AND B.DSPS_RQT_SQNO = C.DSPS_RQT_SQNO AND C.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY AND ROWNUM = 1) $relInspMtrlCnd$ EXISTS (SELECT 1
FROM TB_CSPBII100M B , TB_CSPBII130L C WHERE B.INFO_YR = C.INFO_YR AND B.FRS_MDM_SQNO = C.FRS_MDM_SQNO AND C.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY AND ROWNUM = 1) $relInspMtrlCnd$ EXISTS (SELECT 1
...
FROM TB_BADGDA001M B , TB_BADGDA001L C WHERE B.INST_CD = C.INST_CD AND B.NTFCTN_SECD = C.NTFCTN_SECD AND B.LINK_YR = C.LINK_YR AND B.SRECS_DEPT_CD = C.SRECS_DEPT_CD AND B.NTFCTN_SQNO = C.NTFCTN_SQNO AND C.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY AND ROWNUM = 1) $relInspMtrlCnd$ EXISTS (SELECT 1
...
FROM TBDCMACM015M B WHERE B.ENFM_DPT_CD = #dptCd#) ORDER BY INFO_RLPR_NM