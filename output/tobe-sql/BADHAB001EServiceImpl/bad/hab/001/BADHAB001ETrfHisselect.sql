...
FROM TBBADHAB002H WHERE CMPTN_YN = 'Y' AND TRF_AF_CHR_DPT_CD IN (SELECT SRECS_DEPT_CD /* 심사재심의부서코드 */
FROM TABLE(F_AUTH_DEPT(#strCnb#, #strDptAuth#, '', #strDptCd#))) AND LINK_YR = #LINK_YR# AND TRF_AF_CHR_DPT_CD = #strDptCd2# AND CVLCPT_TASK_SECD = #CVLCPT_TASK_SECD# ORDER BY TRF_DT DESC, FRST_REG_DT DESC