SELECT BZTRP_YR /* 출장년도 */,
    ...
    BZTRVR_NM /* 출장자명 */,
    JBGD_NM /* 직급명 */,
    BLT_BRDV_NM AS CNSTN_MBCMT_DEPT_NM,
    BZTRP_TRVEXP_FDEXP /* 출장여비식비 */,
    BTEXP_CRTR_AMT /* 출장비기준금액 */,
    LDG_CST /* 숙박비용 */,
    CPRG_FARE_CST /* 수도권운임비용 */,
    NVL(BZTRP_TRVEXP_FDEXP, 0) + NVL(BTEXP_CRTR_AMT, 0) + NVL(LDG_CST, 0) AS BZTRP_TRVEXP_FDEXP /* 출장여비식비 */,
    SPM_CST /* 보충비용 */,
    BZTRP_DCN /* 출장일수 */,
    RL_FARE_AMT /* 철도운임금액 */,
    LEAD_CST /* 지휘비용 */,
    ADT_ACTV_ACTC_AMT /* 감사활동수용금액 */,
    SUM_ALL AS SUM_ALL,
    '' AS SORTER
FROM ( SELECT D1.BZTRP_YR AS BZTRP_YR /* 출장년도 */,
    ...
    F_DPT_INFO(D1.OGDP_DEPT_CD, '4') AS F_DPT_INFO /* 소속명 */,
...
FROM ( SELECT BZTRP_YR /* 출장년도 */,
    ...
    BZTRVR_ENO /* 출장자직원전산번호 */,
    MAX(BZTRP_CLCLN_SQNO) AS BZTRP_CLCLN_SQNO /* 출장정산순번 */
FROM TBBADDED019H WHERE BZTRP_YR = #BZTRP_YR# AND BZTRP_SQNO = #BZTRP_SQNO# AND NVL(CLCLN_YN,'N') = 'Y' GROUP BY BZTRP_YR , BZTRP_SQNO , BZTRVR_ENO ) D1 , VADED001 D2 WHERE 1=1 AND D1.BZTRP_YR = D2.BZTRP_YR AND D1.BZTRP_SQNO = D2.BZTRP_SQNO AND D1.BZTRVR_ENO = D2.BZTRVR_ENO AND D1.MAX_SNO = D2.BZTRP_CLCLN_SQNO AND D1.BZTRP_YR = #BZTRP_YR# AND D1.BZTRP_SQNO = #BZTRP_SQNO# ) D2 WHERE 1=1 AND D1.BZTRP_YR = D2.BZTRP_YR(+) AND D1.BZTRP_SQNO = D2.BZTRP_SQNO(+) AND D1.BZTRVR_ENO = D2.BZTRVR_ENO(+) ) WHERE 1=1 AND BTEXP_CRTR_AMT <>0 OR LDG_CST<>0 OR BZTRP_TRVEXP_FDEXP<>0 OR SPM_CST<>0 OR BZTRP_DCN<>0 OR RL_FARE_AMT<>0 OR LEAD_CST<>0 OR ADT_ACTV_ACTC_AMT<>0 OR SUM_COST<>0 OR SUM_ALL<>0 OR CPRG_FARE_CST <> 0 ORDER BY BZTRVR_ENO