...
FROM TB_BADDED003M A ,TB_BADDED001M B WHERE 1=1 AND B.BZTRP_BGT_SECD = '1' /* 확인출장(실국배정)은 제외 - 2015.04.28 yyh, 실국배정 조건 로직 변경(AUD_KND_CD 에서 BZT_BGT_DVSN_CD 로) - 2016.09.29 yyh */ AND A.CVLCPT_ADT_YR(+) = B.ADT_YR AND A.ADT_NO(+) = B.ADT_NO AND SUBSTRING(B.MNG_DEPT_CD, 1, 1) = '0' AND ((A.ACADT_PRCS_STTS_SECD = '1' AND TO_DATE(NVL(A.DAG_YMD ,'99991231')) - TO_DATE(SYSDATE) <= 0) OR A.ACADT_PRCS_STTS_SECD = '3') ) /*부서별*/ SELECT MNG_DEPT_CD AS MNG_DEPT_CD /* 관리부서코드 */,
    ...
    (SELECT AS DPT_SNO
FROM TBDCMACM002M WHERE SRECS_DEPT_CD = MNG_DEPT_CD) AS DPT_ORDER FROM BASE WHERE SUBSTRB(MNG_DEPT_CD, 1, 3) || '000' = #strSpvBrdvCd# GROUP BY MNG_DEPT_CD ORDER BY DPT_ORDER