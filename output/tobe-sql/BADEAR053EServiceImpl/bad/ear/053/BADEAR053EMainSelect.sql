...
FROM TBBADEAR034M T1 ,TB_BADDED001M T2 ,TB_BADEAR001M T3 ,TB_BADEAR002D T4 ,(SELECT CVLCPT_ADT_YR /* 민원감사년도 */,
    ...
    COUNT(1)-1 AS 1
FROM TBBADEAR006M GROUP BY CVLCPT_ADT_YR, ADT_NO, DSPS_RQT_SQNO ) T5 WHERE 1=1 AND T1.CVLCPT_ADT_YR = T2.ADT_YR AND T1.ADT_NO = T2.ADT_NO AND T1.CVLCPT_ADT_YR = T3.ADT_YR AND T1.ADT_NO = T3.ADT_NO AND T1.DSPS_RQT_SQNO = T3.DSPS_RQT_SQNO AND T1.CVLCPT_ADT_YR = T4.ADT_YR AND T1.ADT_NO = T4.ADT_NO AND T1.DSPS_RQT_SQNO = T4.DSPS_RQT_SQNO AND T1.CVLCPT_ADT_YR = T5.CVLCPT_ADT_YR AND T1.ADT_NO = T5.ADT_NO AND T1.DSPS_RQT_SQNO = T5.DSPS_RQT_SQNO /* 결정례 년도 */ AND T1.DCSEXP_YR = #strDcsExpYr# /* 결정례 번호 */ AND T1.DCSEXP_NO = #strDcsExpNo# /* 결정례(처분) 제목 */ AND ( T1.DCSEXP_TIT_NM LIKE '%' || #strDcsExpTitNm# || '%' OR T3.DSPS_RQT_NM LIKE '%' || #strDcsExpTitNm# || '%' ) /* 처분종류 */ AND T3.DSPS_RQT_KDCD = #DSPS_RQT_KDCD# /* 관계기관 */ AND T4.REL_INST_CD = #SRECS_REL_INST_CD# ORDER BY T1.DCSEXP_YR DESC ,T1.DCSEXP_NO DESC