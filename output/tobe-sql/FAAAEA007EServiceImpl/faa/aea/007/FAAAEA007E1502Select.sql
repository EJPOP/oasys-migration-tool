...
FROM TB_BADDED002M WHERE REL_ADT_YR = T1.CVLCPT_ADT_YR AND REL_ADT_NO = T1.ADT_NO ) ) END) AS PRSS_STEP_NM /*진행단계코드명*/ , T1.ADT_BGNG_YMD AS ADT_BGNG_YMD , T1.ADT_END_YMD AS ADT_END_YMD /* 실지감사 시작일, 종료일 - 2017.11.08 EUNOK */ , T1.ACADT_END_YMD AS ACADT_END_YMD /* 종료보고일- 2017.10.31 EUNOK */ , T1.DOC_LIST AS DOC_LIST_A , T1.SLCT_RPTP_NM AS RPRT_NM_A , TO_CHAR(T1.CRU_LAST_ATRZ_DT,'YYYYMMDD') AS CRU_LAST_ATRZ_DT , T1.ATRZ_STTS_SECD FROM ( SELECT D1.ADT_YR AS ADT_YR /* 감사년도 */,
...
FROM TBBADDED103M A1 WHERE A1.ADT_STP_SECD = 'A1050' AND A1.SLCT_RPTP_KDCD = 'A10500500' AND (A1.ATRZ_STTS_SECD IS NULL OR A1.ATRZ_STTS_SECD = '20170908' AND T1.ADT_BGNG_YMD