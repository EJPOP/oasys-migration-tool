...
FROM ( SELECT INFO_YR /* 정보년도 */,
    ...
    PIC_ID /* 담당자아이디 */
FROM TBBADCMR021L ) GROUP BY INFO_YR ,TASK_SNO) T2 WHERE 1=1 AND T1.INFO_YR = T2.INFO_YR(+) AND T1.TASK_SNO = T2.TASK_SNO(+) AND (SELECT COUNT(1)
FROM TBBADCMR017L WHERE INFO_YR = T1.INFO_YR AND TASK_SNO = T1.TASK_SNO AND INST_CD = #INST_CD#) > 0