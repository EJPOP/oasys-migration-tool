...
FROM TBCSPBII100M S WHERE S.PRSNR_ENO = A.TRGT_NOP_ENO AND S.SBMSN_YMD BETWEEN #strStrSmtDt# AND #strEndSmtDt# AND S.SBMSN_YMD = D.SBMSN_YMD ) AS FRS_MDM_SQNO , SBMSN_SCR_RNK FROM TBCSPBII160L A , TBDCMACM001M B , TBDCMACM002M C , (SELECT MIN(SBMSN_YMD) AS SBMSN_YMD /* 제출일자 */,
    PRSNR_ENO /* 제출자직원전산번호 */
FROM TBCSPBII100M WHERE SBMSN_YMD BETWEEN #strStrSmtDt# AND #strEndSmtDt# GROUP BY PRSNR_ENO) D WHERE A.TRGT_NOP_ENO = B.TRGT_NOP_ENO AND A.TRGT_NOP_ENO = D.PRSNR_ENO(+) AND A.INFO_YR = #INFO_YR# AND A.INFO_MLG_QTR_SECD = #INFO_MLG_QTR_SECD# AND nvl(A.SRECS_DEPT_CD,B.SRECS_DEPT_CD) = C.SRECS_DEPT_CD AND A.TRGT_NOP_ENO = #TRGT_NOP_ENO# AND B.SRECS_DEPT_CD = #SRECS_DEPT_CD# B.SRECS_DEPT_CD = #SRECS_DEPT_CD# B.SRECS_DEPT_CD = #SRECS_DEPT_CD# B.SRECS_DEPT_CD = #SRECS_DEPT_CD# ) B.SRECS_DEPT_CD != #SRECS_DEPT_CD# B.SRECS_DEPT_CD != #SRECS_DEPT_CD# B.SRECS_DEPT_CD != #SRECS_DEPT_CD# B.SRECS_DEPT_CD != #SRECS_DEPT_CD# ) AND A.MLG_RMRK_CN LIKE '%'||#strMlgRmk#||'%' AND A.SBMSN_WHOL_NOCS > 0 AND A.SBMSN_WHOL_NOCS AND CASE WHEN C.SRECS_DEPT_CD IN ('062500','062600','062700','062800','170300','170320','071100','071300','180100','180110','100100','081100','081200','081300','081400','081500','081600','081700','081800','081900','0L0100','0L0200','0L0300') THEN 'A' ELSE C.DEPT_TPCD END = #strDPT_TP_CD# AND B.GEN_ADMN_DVSN_CD = #strGEN_ADMN_DVSN_CD# ) AA ORDER BY SBMSN_WHOL_SCR DESC, SMT_DT1 ASC , nvl((SELECT MIN(M.SBMSN_YMD) AS SBMSN_YMD /* 제출일자 */
FROM TB_CSPBII100M M WHERE M.PRSNR_ENO = AA.TRGT_NOP_ENO AND M.SBMSN_YMD BETWEEN #strStrSmtDt# AND #strEndSmtDt# AND M.FRS_MDM_SQNO != AA.FRS_MDM_SQNO ),'0') ASC, JBGD_NM ASC, TRGT_NOP_ENO ASC