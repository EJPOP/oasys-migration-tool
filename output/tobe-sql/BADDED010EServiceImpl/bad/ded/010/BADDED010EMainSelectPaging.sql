SELECT F_MNG_KND_AUDYRNO(D2.CVLCPT_ADT_YR, D2.ADT_NO, D2.ADT_KDCD, '-') ||'('|| D1.CVLCPT_ADT_STP_SQNO || ')' AS ADT_CLSF_NO /* 감사분류번호 */,
    CASE WHEN D2.CVLCPT_ADT_YR >= '2016' THEN SUBSTR(D2.ADT_KDCD,3,1) ELSE '' END AS CVLCPT_ADT_YR /* 민원감사년도 */,
    ...
    D1.CVLCPT_ADT_STP_SQNO AS CVLCPT_ADT_STP_SQNO /* 민원감사단계순번 */,
...
FROM (SELECT DISTINCT AS BZTRP_YR,
    ...
    EMP_SECD /* 직원구분코드 */
FROM TBBADDED005L) A GROUP BY BZTRP_YR, BZTRP_SQNO ) D3 WHERE 1=1 AND D1.BZTRP_SECD = '1' /*감사출장*/ AND D1.REL_ADT_YR = D2.CVLCPT_ADT_YR AND D1.REL_ADT_NO = D2.ADT_NO AND D1.BZTRP_YR = D3.BZTRP_YR(+) AND D1.BZTRP_SQNO = D3.BZTRP_SQNO(+) AND D2.MNG_DEPT_CD IN (SELECT SRECS_DEPT_CD /* 심사재심의부서코드 */
FROM TABLE(F_AUTH_DEPT(#strCnb#, #dptAuth#, '', #strUpSpvBrdvCd#))) /*직급이 원장일 경우 'ALL' 그외는 'DEPTLW'로 셋팅*/ AND D2.MNG_DEPT_CD = #BZTRP_DEPT_CD# AND D1.BZTRP_SECD = #BZTRP_SECD# AND D1.REL_ADT_YR = #REL_ADT_YR# AND D1.REL_ADT_NO = #REL_ADT_NO# /* 감사연번종류코드 - 2015.12.18 yyh */ AND SUBSTR(D2.ADT_KDCD,3,1) = #strAudYrNoKndCd# /* 감사부여번호 추가 - 2016.05.27 yyh */ AND D2.ADT_GRNT_NO = #ADT_GRNT_NO# AND D2.ADT_MTTR_NM LIKE '%'||#strAudSbjNm#||'%' AND D2.ADT_KDCD = #ADT_KDCD# /*감사종류코드*/ AND D1.ADT_BZTRP_KDCD = #ADT_BZTRP_KDCD# /*출장종류코드*/ AND D1.BZTRP_DEPT_CD = #BZTRP_DEPT_CD# /*출장국과코드*/ AND D1.CNTRL_YN = 'Y' AND NVL(D1.CNTRL_YN,'N') = 'N' AND D1.TRSM_YN = 'Y' AND NVL(D1.TRSM_YN,'N') = 'N' AND D1.BZTRP_SQNO = #BZTRP_SQNO# AND D1.BZTRP_YR = #BZTRP_YR# AND D1.CNTRL_TRGT_YN = 'Y' AND D1.TRSM_YN = 'Y' AND D1.DSPS_RQT_NM LIKE #TIT_TXT#||'%'