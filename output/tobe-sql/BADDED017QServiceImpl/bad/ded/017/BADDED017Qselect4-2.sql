...
FROM (SELECT TER.BZTRP_YR /* 출장년도 */,
    ...
    /*일수*/ TE.BZTRP_BGNG_YMD AS BZTRP_BGNG_YMD /* 출장시작일자 */,
    ...
    /*감사활동수용비*/ TE.SPM_CST + ( CASE WHEN SITU.BZTRP_TL_ENO = TER.BZTRVR_ENO THEN TER2.LEAD_CST + TER2.ADT_ACTV_ACTC_AMT ELSE 0 END) AS SPM_CST /* 보충비용 */
...
FROM TBBADDED011L TE0 WHERE BZTRP_YR = #BZTRP_YR# AND BZTRP_SQNO = #BZTRP_SQNO# GROUP BY BZTRP_YR, BZTRP_SQNO, BZTRVR_ENO) TE, (SELECT BZTRP_YR /* 출장년도 */,
    ...
    SUM( CASE WHEN CAL_YN='Y' THEN ADT_ACTV_ACTC_AMT ELSE 0 END) - SUM( CASE WHEN CAL_YN='N' THEN ADT_ACTV_ACTC_AMT ELSE 0 END) AS ADT_ACTV_ACTC_AMT /* 감사활동수용금액 */
