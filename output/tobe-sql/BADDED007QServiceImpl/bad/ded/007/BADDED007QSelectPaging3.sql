...
FROM (select A.BZTRP_YR /* 출장년도 */,
    ...
    C.ADT_MTTR_NM /* 감사사항명 */,
    /* C.AUD_YR || '-' || C.AUD_TASK_CD || '-' || C.ORG_DVSN_CD|| '-' || substr(C.AUD_KND_CD, 3, 1) || '-' || C.SPV_BRDV_CD || '-' || C.AUD_WAY_CD || '-' || C.AUD_NO as AUD_NO */ /* 감사연번호 변경 - 2016.04.08 yyh */ F_MNG_KND_AUDYRNO(C.ADT_YR, C.ADT_NO, C.ADT_KDCD, '-') AS ADT_NO /* 감사번호 */
...
from TBDCMACM013C where CMM_CD_DVSN_CD = '1000001') BBB, (select CD,
    CD_NM
from TBDCMACM013C where CMM_CD_DVSN_CD = '1000007') CCC, TBDCMACM002M DDD, TBDCMACM001M EEE where BBB.CD(+) = AAA.RGN_CD and CCC.CD(+) = AAA.ADT_KDCD and DDD.SRECS_DEPT_CD(+) = AAA.SPRVSN_DEPT_CD and EEE.TRGT_NOP_ENO(+) = AAA.BZTRP_TL_ENO ) B WHERE A.BZTRP_YR = B.BZTRP_YR AND A.INST_CD = B.INST_CD