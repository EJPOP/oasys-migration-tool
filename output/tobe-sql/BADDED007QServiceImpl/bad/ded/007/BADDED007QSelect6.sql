SELECT AAA.*,
    ...
    FFF.CD_NM,
    F_MNG_KND_AUDYRNO(AAA.CVLCPT_ADT_YR, AAA.ADT_NO, AAA.ADT_KDCD, '-') || '(' || CVLCPT_ADT_STP_SQNO || ')' AS F_MNG_KND_AUDYRNO
FROM (SELECT AA.*,
    BB.IN_PSON_CNT,
    BB. AS BB.,
    CC.IN_PSON_MAN_CNT,
...
FROM TBDCMACM013C WHERE CMM_CD_DVSN_CD = '1000007') CCC , TBDCMACM001M DDD , TBDCMACM002M EEE , (SELECT CD,
    CD_NM
FROM TBDCMACM013C WHERE CMM_CD_DVSN_CD = '1000030') FFF WHERE BBB.INST_CD = AAA.INST_CD AND CCC.CD = AAA.ADT_KDCD AND DDD.TRGT_NOP_ENO = AAA.BZTRP_TL_ENO AND EEE.SRECS_DEPT_CD = AAA.SPRVSN_DEPT_CD AND FFF.CD = AAA.ADT_BZTRP_KDCD ORDER BY BZTRP_YR, BZTRP_SQNO