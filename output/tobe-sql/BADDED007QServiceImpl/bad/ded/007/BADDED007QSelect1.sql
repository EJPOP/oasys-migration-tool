...
FROM TBDCMACM013C WHERE CMM_CD_DVSN_CD = '1000007') CCC , TBDCMACM001M DDD , TBDCMACM002M EEE , (SELECT CD,
    CD_NM
FROM TBDCMACM013C WHERE CMM_CD_DVSN_CD ='1000030') FFF WHERE BBB.INST_CD(+) = AAA.INST_CD AND EEE.SRECS_DEPT_CD(+) = AAA.SPRVSN_DEPT_CD AND FFF.CD(+) = AAA.ADT_BZTRP_KDCD AND CCC.CD(+) = AAA.ADT_KDCD AND DDD.TRGT_NOP_ENO = AAA.BZTRP_TL_ENO ORDER BY F_MNG_KND_AUDYRNO(AAA.CVLCPT_ADT_YR, AAA.ADT_NO, '', '-') || '(' || AAA.CVLCPT_ADT_STP_SQNO || ')'