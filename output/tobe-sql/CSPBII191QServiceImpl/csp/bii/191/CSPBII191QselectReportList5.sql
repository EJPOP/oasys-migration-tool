...
FROM ( SELECT NVL2(CD,CD,'총계') AS NVL2,
    ...
    SUM(C8) AS C8,
    SUM(C9) AS C9
FROM ( SELECT SUBSTR(E.SRECS_DEPT_CD, 0, 3) AS SUBSTR,
    E.SRECS_DEPT_CD /* 심사재심의부서코드 */,
    E.ABR_DPT_NM_1 AS CNSTN_MBCMT_DEPT_NM,
    NVL(A.CNT, 0) AS CNT,
...
FROM TBCSPBII100M A WHERE PRCS_RSLT_SECD = '31' AND A.CMPTN_YMD BETWEEN #strStrDt# AND #strEndDt# ) A WHERE 1=1 GROUP BY ETC_LBSVC_MNG_DEPT_CD ) A, ( SELECT A.ETC_LBSVC_MNG_DEPT_CD /* 기타용역관리부서코드 */,
    SUM(A.C1) AS C1,
    SUM(A.A1) AS A1,
    SUM(A.P1) AS P1,
    SUM(A.C2) AS C2,
...
FROM TBBADEAR007M E WHERE B.CVLCPT_ADT_YR = E.CVLCPT_ADT_YR AND B.ADT_NO = E.ADT_NO AND B.DSPS_RQT_SQNO = E.DSPS_RQT_SQNO),0) ELSE 0 END AS P7 FROM TBCSPBII100M A, TBCSPBII120L B, TBBADEAR001M C, TBBADEAR002D D WHERE 1=1 AND A.CMPTN_YMD BETWEEN #strStrDt# AND #strEndDt# AND A.INFO_YR = B.INFO_YR AND A.FRS_MDM_SQNO = B.FRS_MDM_SQNO AND B.CVLCPT_ADT_YR = C.CVLCPT_ADT_YR AND B.ADT_NO = C.ADT_NO AND B.DSPS_RQT_SQNO = C.DSPS_RQT_SQNO AND B.CVLCPT_ADT_YR = D.CVLCPT_ADT_YR AND B.ADT_NO = D.ADT_NO AND B.DSPS_RQT_SQNO = D.DSPS_RQT_SQNO AND A.PRCS_RSLT_SECD IN ('31') ) A WHERE 1=1 GROUP BY ETC_LBSVC_MNG_DEPT_CD ) B , ( SELECT A.ETC_LBSVC_MNG_DEPT_CD /* 기타용역관리부서코드 */,
    COUNT(*) AS C8
FROM ( SELECT A.INFO_YR /* 정보년도 */,
    A.INFO_SQNO /* 정보순번 */,
    A.PRCS_DEPT_CD /* 처리부서코드 */
FROM TBCSPBII100M A WHERE PRCS_RSLT_SECD = '34' AND A.CMPTN_YMD BETWEEN #strStrDt# AND #strEndDt# ) A WHERE 1=1 GROUP BY ETC_LBSVC_MNG_DEPT_CD ) C , ( SELECT A.ETC_LBSVC_MNG_DEPT_CD /* 기타용역관리부서코드 */,
    COUNT(*) AS C9
FROM ( SELECT A.INFO_YR /* 정보년도 */,
    A.INFO_SQNO /* 정보순번 */,
    A.PRCS_DEPT_CD /* 처리부서코드 */
FROM TBCSPBII100M A WHERE PRCS_RSLT_SECD = '32' AND A.CMPTN_YMD BETWEEN #strStrDt# AND #strEndDt# ) A WHERE 1=1 GROUP BY ETC_LBSVC_MNG_DEPT_CD ) D, TBDCMACM002M E WHERE 1=1 AND E.SRECS_DEPT_CD = A.ETC_LBSVC_MNG_DEPT_CD(+) AND E.SRECS_DEPT_CD = B.ETC_LBSVC_MNG_DEPT_CD(+) AND E.SRECS_DEPT_CD = C.ETC_LBSVC_MNG_DEPT_CD(+) AND E.SRECS_DEPT_CD = D.ETC_LBSVC_MNG_DEPT_CD(+) /*AND SUBSTR(E.DPT_CD, 4, 6) != '000' */ ORDER BY CD, SRECS_DEPT_CD ASC ) GROUP BY ROLLUP( CD, SRECS_DEPT_CD) ORDER BY DECODE(CD,'총계','000',CD), DECODE(CNSTN_MBCMT_DEPT_NM,'국별소계','000',SRECS_DEPT_CD) ASC ) WHERE CNT <> 0 OR C0 <> 0 OR P0 <> 0 OR A0 <> 0 OR C8 <> 0 OR C9 <> 0