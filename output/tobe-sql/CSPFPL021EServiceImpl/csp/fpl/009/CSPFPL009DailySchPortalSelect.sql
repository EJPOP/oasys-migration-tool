...
FROM DUAL), 'YYYYMMDD'), 'd') when 1 then '일' when 2 then '월' when 3 then '화' when 4 then '수' when 5 then '목' when 6 then '금' when 7 then '토' end as MIN_DT_SUB from dual) as MIN_DT_SUB FROM TBCSPFPL005M D1 where 1=1 AND D1.WRT_YMD = #WRT_YMD# AND D1.INCD_DVSN_CD = #strManager#