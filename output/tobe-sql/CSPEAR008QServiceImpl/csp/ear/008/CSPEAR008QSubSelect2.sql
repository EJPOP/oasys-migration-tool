...
FROM TBCSPDCP101M A, TBCSPEAR001M B WHERE A.CLM_SECD = '3' AND A.CLM_SECD = B.CLM_SECD (+) AND A.CVLCPT_RCPT_YR = B.CVLCPT_RCPT_YR (+) AND A.CVLCPT_RCPT_RCPT = B.CVLCPT_RCPT_RCPT(+) AND A.CRU_PRCS_PRCS_STTS NOT IN ('10','21') AND TO_CHAR(A.CRU_RCPT_DT,'YYYYMMDD') &gt; #strDTS# AND TO_CHAR(A.CRU_RCPT_DT,'YYYYMMDD') &lt; #strDTE# AND (B.ADT_RSLT_RPLY_YMD &gt; #strDTS# OR TO_CHAR(A.TPOF_DISCON_DT,'YYYYMMDD') &gt; #strDTS#) AND (B.ADT_RSLT_RPLY_YMD &lt; #strDTE# OR TO_CHAR(A.TPOF_DISCON_DT,'YYYYMMDD') &lt; #strDTE#) GROUP BY A.CVLCPT_RCPT_YR ) A UNION ALL SELECT A.CVLCPT_RCPT_YR /* 민원접수년도 */,
    A.ACP_CNT,
    (A.CNT2+A.CNT3+A.CNT4+A.CNT5+A.CNT55+A.CNT6+A.CNT7) AS CNT2,
    A.CNT2,
    A.CNT3,
