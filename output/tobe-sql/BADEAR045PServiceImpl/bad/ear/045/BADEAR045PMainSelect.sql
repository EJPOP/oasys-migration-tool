SELECT TO_CHAR(TO_DATE(D2.ENFC_YMD),'YYYY. MM. DD') AS TO_CHAR /* 실행일자 */,
    ...
    D5.JRSD_INST_CD AS JRSD_INST_CD /* 소관기관코드 */,
...
FROM TBBADEAR006M GROUP BY CVLCPT_ADT_YR , ADT_NO, DSPS_RQT_SQNO ) D3 , TB_BADEAR005M D4 , ( SELECT CVLCPT_ADT_YR AS CVLCPT_ADT_YR /* 민원감사년도 */,
    ADT_NO AS ADT_NO /* 감사번호 */,
    DSPS_RQT_SQNO AS DSPS_RQT_SQNO /* 처분요구순번 */,
    MIN(SRECS_REL_INST_CD) AS JRSD_INST_CD /* 소관기관코드 */,
    F_ORG_INFO(MIN(SRECS_REL_INST_CD),'1') AS F_ORG_INFO
FROM TBBADEAR008M GROUP BY CVLCPT_ADT_YR , ADT_NO, DSPS_RQT_SQNO ) D5 WHERE 1=1 AND D1.ADT_YR = D2.ADT_YR AND D1.ADT_NO = D2.ADT_NO AND D1.DSPS_RQT_SQNO = D2.DSPS_RQT_SQNO AND D1.ADT_YR = D3.CVLCPT_ADT_YR(+) AND D1.ADT_NO = D3.ADT_NO(+) AND D1.DSPS_RQT_SQNO = D3.DSPS_RQT_SQNO(+) AND D1.ADT_YR = D4.ADT_YR(+) AND D1.ADT_NO = D4.ADT_NO(+) AND D1.DSPS_RQT_SQNO = D4.DSPS_RQT_SQNO(+) AND D1.ADT_YR = D5.CVLCPT_ADT_YR(+) AND D1.ADT_NO = D5.ADT_NO(+) AND D1.DSPS_RQT_SQNO = D5.DSPS_RQT_SQNO(+) AND D1.ADT_YR = #CVLCPT_ADT_YR# AND D1.ADT_NO = #ADT_NO# AND D1.DSPS_RQT_SQNO = #DSPS_RQT_SQNO#