...
FROM TB_BADDED001M S1 WHERE S1.CVLCPT_ADT_YR = SUBSTR(D1.ADT_SQNO, 0, 4) AND S1.ADT_NO = SUBSTR(D1.ADT_SQNO, 5, 3)) AS ADT_GRNT_NO ,NVL(D1.PBRL_SBMSN_YN,'N') AS PBRL_SBMSN_YN ,TO_CHAR(TO_DATE(D1.PBRL_SBMSN_YMD),'YYYY-MM-DD') AS PBRL_SBMSN_YMD ,D1.RLS_CYCL /* 본안,별도건의 시행이 다를경우 최종 시행일만 가져오는 오류 수정 2021.07.09 EUNOK ,(SELECT TO_CHAR(TO_DATE(MAX(ENF_DT)),'YYYY-MM-DD') FROM TBBADEAR002D S1 WHERE S1.AUD_YR = D2.AUD_YR AND S1.AUD_NO = D2.AUD_NO) AS ENF_DT */ ,(SELECT TO_CHAR(TO_DATE(MIN(C.ENFC_YMD)), 'YYYY-MM-DD') AS ENFC_YMD /* 시행일자 */
FROM TB_BADDED146M A INNER JOIN TB_BADEAR001M B ON (B.CVLCPT_ADT_YR = A.ADT_YR AND B.ADT_NO = A.ADT_NO AND B.ADT_DSPS_RQT_SQNO = A.DSPS_RQT_SQNO) INNER JOIN TB_BADEAR002D C ON (C.CVLCPT_ADT_YR = B.CVLCPT_ADT_YR AND C.ADT_NO = B.ADT_NO AND C.DSPS_RQT_SQNO = B.DSPS_RQT_SQNO) WHERE A.ADT_YR = D2.ADT_YR AND A.ADT_NO = D2.ADT_NO AND A.CMPTN_YN = 'Y' AND NVL(A.LAST_ADT_RPTP_RFRNC_DOC_ID, A.DOC_ID) IN (SELECT ST.RFRNC_DOC_ID /* 참조문서아이디 */
FROM TB_BADEAR028L ST WHERE ST.ADT_SQNO = D1.ADT_SQNO AND ST.CVLCPT_DOC_ID = D1.#N/A)) AS ENFC_YMD ,TO_CHAR(TO_DATE(D1.PBRL_SBMSN_YMD),'YYYY-MM-DD') AS PBRL_SBMSN_YMD ,CASE WHEN D1.REG_YMD IS NOT NULL THEN TO_DATE(D1.REG_YMD) - (SELECT TO_DATE(MIN(C.ENFC_YMD)) AS ENFC_YMD /* 시행일자 */
FROM TB_BADDED146M A INNER JOIN TB_BADEAR001M B ON (B.CVLCPT_ADT_YR = A.ADT_YR AND B.ADT_NO = A.ADT_NO AND B.ADT_DSPS_RQT_SQNO = A.DSPS_RQT_SQNO) INNER JOIN TB_BADEAR002D C ON (C.CVLCPT_ADT_YR = B.CVLCPT_ADT_YR AND C.ADT_NO = B.ADT_NO AND C.DSPS_RQT_SQNO = B.DSPS_RQT_SQNO) WHERE A.ADT_YR = D2.ADT_YR AND A.ADT_NO = D2.ADT_NO AND A.CMPTN_YN = 'Y' AND NVL(A.LAST_ADT_RPTP_RFRNC_DOC_ID, A.DOC_ID) IN (SELECT ST.RFRNC_DOC_ID /* 참조문서아이디 */
FROM TB_BADEAR028L ST WHERE ST.ADT_SQNO = D1.ADT_SQNO AND ST.CVLCPT_DOC_ID = D1.#N/A)) ELSE TO_DATE(SYSDATE) - (SELECT TO_DATE(MIN(C.ENFC_YMD)) AS ENFC_YMD /* 시행일자 */
FROM TB_BADDED146M A INNER JOIN TB_BADEAR001M B ON (B.CVLCPT_ADT_YR = A.ADT_YR AND B.ADT_NO = A.ADT_NO AND B.ADT_DSPS_RQT_SQNO = A.DSPS_RQT_SQNO) INNER JOIN TB_BADEAR002D C ON (C.CVLCPT_ADT_YR = B.CVLCPT_ADT_YR AND C.ADT_NO = B.ADT_NO AND C.DSPS_RQT_SQNO = B.DSPS_RQT_SQNO) WHERE A.ADT_YR = D2.ADT_YR AND A.ADT_NO = D2.ADT_NO AND A.CMPTN_YN = 'Y' AND NVL(A.LAST_ADT_RPTP_RFRNC_DOC_ID, A.DOC_ID) IN (SELECT ST.RFRNC_DOC_ID /* 참조문서아이디 */
FROM TB_BADEAR028L ST WHERE ST.ADT_SQNO = D1.ADT_SQNO AND ST.CVLCPT_DOC_ID = D1.#N/A)) END AS DAY_CNT /*시행일 이후 경과일수*/ ,F_MNG_KND_AUDYRNO(D2.ADT_YR, D2.ADT_NO, D2.ADT_KDCD, '-') AS ADT_CLSF_NO ,DECODE(D1.RLS_CYCL,NULL,NULL,'홈페이지 공개문 '||D1.RLS_CYCL||'차') AS DGE_NM ,D1.WRTR_ENO ,F_USR_INFO(D1.WRTR_ENO, '2') AS WRTR_NM ,NVL(D1.UDC_YN,'N') AS UDC_YN , CASE WHEN D1.RLS_YN = 'Y' AND (D1.UDC_YN = 'N' OR D1.UDC_YN IS NULL) THEN '홈페이지공개' WHEN D1.RLS_YN = 'Y' AND D1.UDC_YN = 'Y' THEN '미공개종결' WHEN D1.PRGRS_STTS_SECD = '99' THEN '홍보등록' ELSE '제출완료' END AS OPEN_NM ,D1.PRGRS_STTS_SECD ,D1.RMRK_CN ,F_DPT_INFO(D2.SPRVSN_DEPT_CD,'1') AS WRIT_DPT_NM /*담당(제출)과*/ FROM TB_BADEAR028M D1 ,TB_BADDED001M D2 WHERE 1=1 AND D1.TASK_SECD = 'KP1' AND D2.ADT_YR = SUBSTR(D1.ADT_SQNO, 0, 4) AND D2.ADT_NO = SUBSTR(D1.ADT_SQNO, 5, 3) AND D1.PBRL_SBMSN_YN = 'Y' AND ((TO_CHAR(TO_DATE(D1.REG_YMD, 'YYYYMMDD'), 'YYYYMMDD') BETWEEN #STARTDT# AND #ENDDT#) OR (D1.PBRL_SBMSN_YMD BETWEEN #STARTDT# AND #ENDDT#)) AND D1.ADT_FLD_SECD = #ADT_FLD_SECD# AND D1.PRCS_DEPT_CD = #ETC_LBSVC_MNG_DEPT_CD# AND D1.REL_INST_CD = #REL_INST_CD# AND D1.RLS_YN = #RLS_YN# AND D1.RLS_CYCL IS NULL AND D1.RLS_CYCL IS NOT NULL AND ( D1.ADT_MTTR_NM LIKE '%'||#SEARCHVALUE#||'%' OR D1.ADT_MTTR_CN LIKE '%'||#SEARCHVALUE#||'%' ) AND D1.ADT_MTTR_NM LIKE '%'||#SEARCHVALUE#||'%' AND D1.ADT_MTTR_CN LIKE '%'||#SEARCHVALUE#||'%' ORDER BY D1.TASK_SQNO DESC