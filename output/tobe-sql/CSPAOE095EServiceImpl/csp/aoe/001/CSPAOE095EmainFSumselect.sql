SELECT /* CSPAOE095EmainFSumselect */ ROUND(AVG(PSN_ASC), 3) AS ROUND /* 평점 평균 */,
    ROUND(STDDEV(PSN_ASC), 3) AS ROUND /* 평점 표준편차 */
FROM (SELECT A.SRECS_DEPT_CD /* 심사재심의부서코드 */,
    A.DPT_ASC /* 부서평점 */,
    A.DPT_NOP_CNT /* 부서인원수 */,
    ROUND(A.DPT_ASC / A.DPT_NOP_CNT, 2) AS ROUND /* 개인평점 */,
    ROW_NUMBER() OVER(ORDER BY A.DPT_ASC / A.DPT_NOP_CNT ASC) AS ROW_NUMBER /* 최저 점수 부터 순위 */,
    ROW_NUMBER() OVER(ORDER BY A.DPT_ASC / A.DPT_NOP_CNT DESC) AS ROW_NUMBER /* 최대 점수 부터 순위 */
FROM (SELECT BASE.EVL_YR,
    BASE.HLYR_DVSN_CD,
...
FROM TBCSPAOE035M WHERE EVL_YR = #strEvlYr# AND HLYR_DVSN_CD = #strHlyrDvsnCd# AND CVLCPT_TASK_SECD = '10001' UNION ALL SELECT BLT_DPT_CD AS SRECS_DEPT_CD /* , ROUND((DRIVE_RTE_ASC + ASP_STND_ASC + SPE_ADD_SC + MDT_ASC) * (CTB_RTE/100), 2) AS PSN_TSK_ASC_1 개인평점 */,
