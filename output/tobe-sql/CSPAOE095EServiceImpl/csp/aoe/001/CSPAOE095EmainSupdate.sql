UPDATE /*+ bypass_ujvc */ ( SELECT D7.EVL_YR,
    ...
    D7.DPT_NOP_CNT_NEW * ROUND(D7.PSO_PER_AV_ASC, 2) AS DPT_NOP_CNT_NEW /* 부서총점 */,
    ROUND(D7.PSO_PER_AV_ASC, 2) AS ROUND /* 인당평균평점 */,
    ROUND(D7.DPT_ASC, 2) AS ROUND /* 평점 */,
    D7.DPT_NOP_CNT_NEW,
    #AVG_SUPP# AS #AVG_SUPP#,
    #STD_DEV_SUPP# AS #STD_DEV_SUPP#,
...
FROM ( SELECT D5.EVL_YR,
    ...
    D5.SRECS_DEPT_CD /* 심사재심의부서코드 */,
    #DPT_NOP_CNT# AS #DPT_NOP_CNT# /* 부서인원 */,
...
FROM TBCSPAOE031M D1 WHERE 1 = 1 AND EVL_STEP_CD = '500' /*위원평가*/ AND EVL_DTM_YN = 'Y' /*평가확정*/ AND EVL_YR = #EVL_YR# AND HLYR_DVSN_CD = #HLYR_DVSN_CD# AND SRECS_DEPT_CD = #SRECS_DEPT_CD# GROUP BY EVL_YR, HLYR_DVSN_CD, SRECS_DEPT_CD, EVL_CMTR_CNB ) D2 GROUP BY EVL_YR, HLYR_DVSN_CD, SRECS_DEPT_CD ) D3 ) D5 ) D7 , TBCSPAOE034M D8 WHERE D7.EVL_YR = D8.EVL_YR AND D7.HLYR_DVSN_CD = D8.HLYR_DVSN_CD AND D7.DPT_TYP_DVSN_CD = D8.DPT_TYP_DVSN_CD AND D7.SRECS_DEPT_CD = D8.SRECS_DEPT_CD ) SET DPT_TSC = DPT_TSC_NEW ,  PSO_PER_AV_ASC = PSO_PER_AV_ASC_NEW ,  DPT_ASC = DPT_ASC_NEW ,  DPT_NOP_CNT = DPT_NOP_CNT_NEW ,  PSO_PER_ASC_AV_VAL = PSO_PER_ASC_AV_VAL_NEW ,  PSO_PER_ASC_STND_DEV_VAL = PSO_PER_ASC_STND_DEV_VAL_NEW ,  LAST_USER_ID = FNL_USR_ID_NEW /* 최종사용자아이디 */,  LAST_PRCS_DEPT_CD = FNL_DEAL_DPT_CD_NEW /* 최종처리부서코드 */,  LAST_MENU_ID = FNL_MNU_ID_NEW /* 최종메뉴아이디 */,  LAST_MDFCN_DT = FNL_MDF_DH_NEW /* 최종수정일시 */