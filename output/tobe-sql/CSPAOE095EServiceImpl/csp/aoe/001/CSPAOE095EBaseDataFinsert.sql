INSERT INTO TBCSPAOE034M (EVL_YR, HLYR_DVSN_CD, DPT_TYP_DVSN_CD, SRECS_DEPT_CD, DPT_ASC, DPT_NOP_CNT, PSN_ASC, FRST_USER_ID, LAST_USER_ID, LAST_PRCS_DEPT_CD, LAST_MENU_ID, FRST_REG_DT, LAST_MDFCN_DT) SELECT #strEvlYr# /* 평가년도 */,
    #strHlyrDvsnCd# /* 반기구분코드 */,
    '10' /* 부서유형구분코드 */,
    JUMSU.SRECS_DEPT_CD /* 심사재심의부서코드 */,
    JUMSU.DPT_ASC /* 부서평점 */,
    NVL(CNT.DPT_NOP_CNT, 1) /* 부서인원수 */,
    ROUND(JUMSU.DPT_ASC / NVL(CNT.DPT_NOP_CNT, 1), 2) AS ROUND /* 개인평점 */,
...
FROM DUAL) AS DRIVE_RTE_ASC /*추진율 조정평점*/ , D10.DLY_DT_SUM /*지연일*/ , NVL(D10.ASP_RTE, 0) AS ASP_RTE /*가감점율*/ , D11.CTB_RT , D11.BLT_DPT_CD , D10.ADT_KDCD , D10.AUD_STEP_CD_MAX , NVL(D10.SPE_ADD_SC, F_AOE_SPE_ADD_SC(#strEvlYr#, #strHlyrDvsnCd#, D10.CVLCPT_ADT_YR, D10.TSK_NO, D10.ADT_KDCD)) AS SPE_ADD_SC , NVL(D10.MDT_ASC, F_AOE_MDT_ASC(#strEvlYr#, #strHlyrDvsnCd#, D10.CVLCPT_ADT_YR, D10.TSK_NO, D10.ADT_KDCD)) AS MDT_ASC , D10.CVLCPT_ADT_YR , D10.TSK_NO FROM ( SELECT ADT_KDCD /* 감사종류코드 */,
...
FROM TBCSPAOE012M WHERE DLAY_DT_NM = TO_CHAR(D9.DLY_DT_SUM) AND EVL_YR = #strEvlYr# AND HLYR_DVSN_CD = #strHlyrDvsnCd#) AS ASP_RTE /*가감점율*/ , EVL_YR , HLYR_DVSN_CD , SYT_TRTM_YN , SPE_ADD_SC , MDT_ASC FROM ( SELECT D5.ADT_KDCD /* 감사종류코드 */,
    ...
    D5.AUD_STEP_CD_MAX,
    D5.MNDY_CNT,
    (SELECT F_AOE_STND_ASC(#strEvlYr#, #strHlyrDvsnCd#, D5.ADT_KDCD, D5.AUD_STEP_CD_MIN, D5.AUD_STEP_CD_MAX, D5.MNDY_CNT, NVL(D5.SYT_TRTM_YN,'N')) AS ADT_KDCD /* 감사종류코드 */
...
FROM TBDCMACM002M WHERE USE_YN = 'Y' /*감사부서만 나온다*/) D23 WHERE D22.SRECS_DEPT_CD = D23.SRECS_DEPT_CD ) GROUP BY SRECS_DEPT_CD ) JUMSU , ( SELECT SRECS_DEPT_CD /* 심사재심의부서코드 */,
    COUNT(TRGT_NOP_ENO) AS TRGT_NOP_ENO /* 대상인원직원전산번호 */
FROM TBDCMACM001M GROUP BY SRECS_DEPT_CD ) CNT /*처음에는 인사정보테이블에서 건수를 가져온다.*/ WHERE JUMSU.SRECS_DEPT_CD = CNT.SRECS_DEPT_CD (+)