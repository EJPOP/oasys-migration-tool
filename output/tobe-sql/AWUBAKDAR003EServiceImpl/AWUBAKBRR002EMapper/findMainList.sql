/* AWU.BAK.BRR.AWUBAKBRR002EMAPPER.findMainList */ SELECT FRS_MDM_SQNO /* 포렌식매체순번 */,
    ...
    COUNT(1) OVER () AS 1
FROM TBAWUBAKBRR002M T1 WHERE 1=1 AND T1.SRECS_DCSN_SECD IN ('1','4') AND T1.SRECS_DCSN_YR = #{srchDcsYr} AND T1.DCS_NO = #{srchDcsNo} AND SRECS_DCSN_SECD = #{srchDcsDvsnCd} AND REQ_DVSN_CD = #{srchReqDvsnCd} AND ( RSCH_ASMT_TTL LIKE '%'||#{srchTitOrIsuTxt}||'%' OR ISU_TXT LIKE '%'||#{srchTitOrIsuTxt}||'%' ) ORDER BY T1.FRS_MDM_SQNO DESC