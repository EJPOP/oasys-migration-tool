SELECT A.INFO_RLPR_NM /* 정보관계자명 */,
    ...
    F_CODE_NM('1000058', A.WRK_RANK_CD) AS F_CODE_NM /* 근무직급명 */,
    (SELECT B.PROF_RBPRSN_INST_NM AS PROF_RBPRSN_INST_NM /* 증명책임자기관명 */
FROM TBDCMACM015M B WHERE B.INST_CD = A.OGDP_INST_CD) AS TPOF_OGDP_INST_NM /* 소속기관명 */ , (SELECT COUNT(*)
FROM TBCSPBII620L B WHERE B.CRU_RRNO_SCRTY = A.CRU_RRNO_SCRTY) AS FMY_CNT /* 가족수 */ , MIN(RRN_SRNO) OVER (PARTITION BY CRU_RRNO_SCRTY) AS RRN_SRNO /* 주민등록번호일련번호 */ , TOT_CNT /* 총건수 */ FROM (SELECT T.*,
    ROWNUM
FROM (SELECT DT.*,
    COUNT(1) OVER() AS 1
FROM (SELECT A.INFO_RLPR_NM /* 정보관계자명 */,
    ...
    A.JOCP_DT /* 입사일 */,
    A.RTRM_YMD /* 퇴직일자 */,
    TO_CHAR(A.FRST_REG_DT, 'YYYYMMDD') AS FRST_REG_DT /* 최초등록일시 */,
    A.INST_SECD /* 기관구분코드 */,
    A.EPS_MNG_DVSN_CD /* 중점관리구분코드 */,
    A.RSG_YN /* 퇴직여부 */,
    A.CRU_RRNO_SCRTY /* 부패주민등록번호보안 */,
    A.RRN_SRNO /* 주민등록번호일련번호 */
FROM TBCSPBII600M A /* 공무원_공공직원 기본 */ WHERE 1=1 AND A.INFO_RLPR_NM LIKE #nam#||'%' AND A.CRU_RRNO_SCRTY LIKE #strEin# || '%' AND A.MTRL_DVSN_CD = #mtrlDvsnCd# AND A.CRU_RRNO_SCRTY IN (SELECT B.CRU_RRNO_SCRTY /* 부패주민등록번호보안 */
...
FROM TBCSPBII631C B , TBCSPBII630L C WHERE B.UNQ_TXT LIKE '%'||#schTxt#||'%' AND B.UNQ_CD = C.UNQ_CD) ORDER BY FRST_REG_DT DESC, RRN_SRNO ) DT ) T ) A WHERE NUM BETWEEN (((#currPage#- 1) * #pageSize#) + 1) AND (#currPage# * #pageSize#)