SELECT /* csp.bii.service.impl.CSPBII216QServiceImpl.CSPBII216QMainSelect */ T1.CVLCPT_ADT_YR AS CVLCPT_ADT_YR /* 민원감사년도 */,
...
FROM TBBADDED001M WHERE 1=1 AND CVLCPT_ADT_YR = D1.CVLCPT_ADT_YR AND ADT_NO = D1.ADT_NO AND SUBSTR(ADT_KDCD,3,1) = '9' AND NVL(SUBSTR(ADT_MTHD_SECD,3,1),' ') <> '4' ) ) T1 WHERE 1=1 AND T1.ADT_BGNG_YMD >= '20170908' AND T1.ADT_BGNG_YMD AND TO_CHAR(T1.CRU_LAST_ATRZ_DT,'YYYYMMDD') &gt;= #strAudEnd_DTS# AND TO_CHAR(T1.CRU_LAST_ATRZ_DT,'YYYYMMDD') &lt;= #strAudEnd_DTE# AND T1.CVLCPT_ADT_YR||LPAD(T1.ADT_GRNT_NO,4,'0') >= #strAudYrSt#||DECODE(#strAudGrnNoSt#,'','0000', LPAD(#strAudGrnNoSt#,4,'0')) AND T1.CVLCPT_ADT_YR||LPAD(T1.ADT_GRNT_NO,4,'0') /* 감사연번종류코드 추가 - 2015.12.22 yyh */ AND SUBSTR(T1.ADT_KDCD,3,1) = SUBSTR(#strAudYrNoKndCdSt#,3,1)