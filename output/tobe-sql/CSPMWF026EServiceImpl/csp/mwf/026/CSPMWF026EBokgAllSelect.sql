...
FROM DUAL), 'YYYYMMDD') AS DT FROM DUAL , BASE_DATE_CTE CONNECT BY LEVEL <= 12 ) ), BASE2 AS ( /* 기준일자와 시간표 */ SELECT DT,
    B.*
FROM ( SELECT TO_CHAR(TO_DATE(BASE_DATE_CTE.BASE_DATE) - 7 + LEVEL + (SELECT NEXT_DAY(TO_DATE(BASE_DATE_CTE.BASE_DATE),1) - TO_DATE(BASE_DATE_CTE.BASE_DATE)
FROM DUAL), 'YYYYMMDD') AS DT FROM DUAL , BASE_DATE_CTE CONNECT BY LEVEL <= 12 ) A ,TBCSPMWF009M B WHERE B.BOKG_DVSN_CD = '1' AND (SELECT COUNT(1)
FROM TB_BADDED013B WHERE INFO_YR || HLDY_CRTR_MND = DT) = 0 /* 휴일제외 */ AND (SELECT COUNT(1)
FROM TBCSPMWF011M WHERE HLDY = A.DT AND BOKG_DVSN_CD = '1') = 0 /* 임시휴일 적용 - 2019.07.29 yyh */ AND TO_CHAR(TO_DATE(DT), 'DY', 'NLS_DATE_LANGUAGE=korean') IN ('화','수','목') /* 화,목 만 조회. 수요일추가 - 2019.03.27 yyh */ ) SELECT TO_CHAR(TO_DATE(A.DT), 'YYYY-MM-DD') || '(' || TO_CHAR(TO_DATE(A.DT), 'DY', 'NLS_DATE_LANGUAGE=korean') || ')' AS DSPS_YMD /* 처분일자 */,
...
FROM TBCSPMWF008M T1 ,TBCSPMWF009M T2 ,BASE T3 ,TBCSPMWF007M T4 ,TBDCMACM001M T5 WHERE 1=1 AND T1.SCO_HST_SNO = T2.SCO_HST_SNO AND T1.SCO_SNO = T2.SCO_SNO AND T2.BOKG_DVSN_CD = '1' AND T1.BOKG_APL_DH = T4.BOKG_APL_DH(+) AND T1.BOKG_PEN_CNB = T5.TRGT_NOP_ENO AND T1.BOKG_DT BETWEEN T3.BGNG_YMD AND T3.END_YMD ) B WHERE 1=1 AND A.BOKG_DVSN_CD = B.BOKG_DVSN_CD(+) AND A.SCO_HST_SNO = B.SCO_HST_SNO(+) AND A.SCO_SNO = B.SCO_SNO(+) AND A.DT = B.BOKG_DT(+) ORDER BY A.DT ,A.SCO_HST_SNO ,A.SCO_SNO