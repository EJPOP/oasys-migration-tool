/* faa/cav/048/FAACAV048ERselect2 */ WITH MST AS( SELECT 'A' AS GUBUN,
    ACNTG_YR /* 회계년도 */,
    JRSD_CD /* 소관코드 */,
    ACNTG_CD /* 회계코드 */,
    TCODE,
    ...
    JINGSU_AMT,
    SUM(JINGSU_AMT - CLCTN_DCSN_AMT) AS JINGSU_AMT
FROM (SELECT A.ACNTG_YR /* 회계년도 */,
    ...
    A.DITM_CD /* 세항코드 */,
    A.SITM_CD /* 목코드 */,
    B.CLCTN_DCSN_AMT /* 징수결정금액 */,
    SUM(A.#N/A) AS #N/A /* 전체결산금액1 */
FROM TWFAACAV061L A, TWFAACAV038L B WHERE A.ACNT_YR = B.ACNT_YR AND A.DTIL_COPT_CD = B.COPT_CD AND A.ACNT_CD = B.ACNT_CD AND A.ACCT_CD = B.ACCT_CD AND A.SSEC_CD = LTRIM(RTRIM(B.SSEC_CD)) AND A.DITM_CD = LTRIM(RTRIM(SUBSTR(B.DITM_CD,1,2))) AND A.SITM_CD = B.SITM_CD AND A.ACNT_YR = #strYr# AND A.SOA_SECD = 'B' AND (A.#N/A != 0 OR B.CLCTN_DCSN_AMT != 0) GROUP BY A.ACNTG_YR, A.DTL_JRSD_SECD, A.ACNTG_CD, A.ACNT_CD, A.SSEC_CD, A.DITM_CD, A.SITM_CD, B.CLCTN_DCSN_AMT ) M1 WHERE M1.CLCTN_DCSN_AMT != M1.JINGSU_AMT GROUP BY ACNTG_YR, JRSD_CD, ACNTG_CD, TCODE, TKWAMOK, ACNT_CD, SSEC_CD, DITM_CD, SITM_CD, CLCTN_DCSN_AMT, JINGSU_AMT UNION SELECT 'B' AS GUBUN,
    ACNTG_YR /* 회계년도 */,
    JRSD_CD /* 소관코드 */,
    ACNTG_CD /* 회계코드 */,
    TCODE,
...
FROM TWFAACAV038L B WHERE A.ACNTG_YR = B.ACNTG_YR AND A.DTL_JRSD_SECD = B.JRSD_CD AND A.ACNTG_CD = B.ACNTG_CD AND A.ACNT_CD = B.ACNT_CD AND A.SSEC_CD = LTRIM(RTRIM(B.SSEC_CD)) AND A.DITM_CD = LTRIM(RTRIM(SUBSTR(B.DITM_CD,1,2))) AND A.SITM_CD = B.SITM_CD) GROUP BY A.ACNTG_YR, A.DTL_JRSD_SECD, A.ACNTG_CD, A.ACNT_CD, A.SSEC_CD, A.DITM_CD, A.SITM_CD ) M2 GROUP BY ACNTG_YR, JRSD_CD, ACNTG_CD, TCODE, TKWAMOK, ACNT_CD, SSEC_CD, DITM_CD, SITM_CD, CLCTN_DCSN_AMT, JINGSU_AMT UNION SELECT 'C' AS GUBUN,
    ...
    CLCTN_DCSN_AMT /* 징수결정금액 */,
    JINGSU_AMT,
    -CLCTN_DCSN_AMT AS CLCTN_DCSN_AMT /* 징수결정금액 */
FROM (SELECT A.ACNTG_YR /* 회계년도 */,
    A.JRSD_CD /* 소관코드 */,
    A.ACNTG_CD /* 회계코드 */,
