...
FROM TBBADDED002M WHERE REL_ADT_YR = #CVLCPT_ADT_YR# AND REL_ADT_NO = #ADT_NO# AND ADT_BZTRP_KDCD IN ('01','02')) AND A.REL_ADT_YR = #CVLCPT_ADT_YR# AND A.REL_ADT_NO = #ADT_NO# ) A group by REL_ADT_YR, REL_ADT_NO ) C WHERE A.CVLCPT_ADT_YR = C.REL_ADT_YR AND A.ADT_NO = C.REL_ADT_NO AND A.CVLCPT_ADT_YR = #CVLCPT_ADT_YR# AND A.ADT_NO = #ADT_NO# ) D7 WHERE ROWNUM = 1 AND D1.CVLCPT_ADT_YR = D2.CVLCPT_ADT_YR (+) AND D1.TSK_NO = D2.TSK_NO (+) AND D1.AUD_LDR_CNB = D3.TRGT_NOP_ENO (+) AND D1.SPVR_ENO = D4.TRGT_NOP_ENO (+) AND D1.ASTN_ENO = D5.TRGT_NOP_ENO (+) AND D1.CVLCPT_ADT_YR = D7.CVLCPT_ADT_YR (+) AND D1.TSK_NO = D7.TSK_NO (+) AND D1.CVLCPT_ADT_YR = #CVLCPT_ADT_YR# AND D1.TSK_NO = #ADT_NO#