...
FROM TBCSPAOE017D WHERE EVL_YR = #strEvlYr# AND HLYR_DVSN_CD = #strHlyrDvsnCd# AND CVLCPT_TASK_SECD = #CVLCPT_TASK_SECD# AND AUD_TSK_KND_CD = #strAudTskKndCd# AND CHGR_DVSN_CD = 'CD02' ) AS EVL_CMTR_NO , NVL(D3.SRECS_DEPT_CD, D4.BLT_DPT_CD) AS SRECS_DEPT_CD , D3.JBGD_SECD , #FRT_USR_ID# , #FNL_USR_ID# , #FNL_DEAL_DPT_CD# , #FNL_MNU_ID# , SYSDATE , SYSDATE FROM ( SELECT 'CD01',
    D1.TRGT_NOP_ENO /* 대상인원직원전산번호 */,
    D1.SRECS_DEPT_CD /* 심사재심의부서코드 */,
    D1.JBGD_SECD /* 직급구분코드 */
FROM TBDCMACM001M D1 , TBDCMACM002M D2 WHERE D1.SRECS_DEPT_CD = D2.SRECS_DEPT_CD AND D2.DEPT_TPCD = 'S' /*지원부서*/ AND D2.USE_YN = 'Y' AND D1.JBPS_CD IN ('0035100', '0067400', '0058100', '0015800', '0013900', '0020600', '0037500') /*단장, 원장, 실장, 관리관, 국장, 기획조정실장(실장)(기획실장), 대변인(국장)이익현 15.01.28 최원형 감사관 요청.*/ AND D1.SRECS_DEPT_CD NOT IN ('770770', '500000' ) /*감사원, 감사원장 평가대상제외부서 15.01.29 최원형감사관님 요청*/ AND D1.USER_NM NOT LIKE 'test%' /*test로 들어간 정보들은 제거한다*/ ) D3 FULL OUTER JOIN ( SELECT BLT_DPT_CD
FROM TBCSPAOE017D WHERE EVL_YR = #strCopyEvlYr# AND HLYR_DVSN_CD = #strCopyHlyrDvsnCd# AND CVLCPT_TASK_SECD = '10002' AND AUD_TSK_KND_CD = '10000' AND CHGR_DVSN_CD = 'CD02' ) D4 ON D3.SRECS_DEPT_CD = D4.BLT_DPT_CD